...
use crate::gate_c::{
    error::{PipelineError, GateCResult},
    types::{PipelinePlan, PipelineStep, StepId, DependencyGraph},
    limits::{MAX_PIPELINE_STEPS, MAX_DEP_GRAPH_EDGES, MAX_DEPENDENCY_DEPTH},
};
    ...
    fn validate_graph(&self, graph: &DependencyGraph) -> GateCResult<()> {
        ...
        if depth > self.max_depth {
            return Err(PipelineError::AmbiguousOrdering(format!(
                "Dependency depth {} exceeds maximum {}", depth, self.max_depth
            )).into());
        }
    ...
    fn calculate_depth_from_node(
        ...
        for (from, to) in &graph.edges {
            if from == node && !visited.contains(to) {
                let depth = self.calculate_depth_from_node(graph, to, visited, current_depth + 1)?;
                max_depth = max_depth.max(depth);
            }
        }
...
impl PipelinePlanner {
    ...
    pub fn plan(&self, steps: Vec<PipelineStep>) -> GateCResult<PipelinePlan> {
        ...
        let created_at = crate::gate_c::deterministic::deterministic_timestamp_from_plan_id("pipeline_plan");
        ...
        self.canonicalizer.validate_ordering(&canonical_order, &dependencies)?;
...
impl OrderCanonicalizer {
    ...
    pub fn validate_ordering(&self, order: &[StepId], graph: &DependencyGraph) -> GateCResult<()> {
        ...
        let position_map: HashMap<_, _> = order.iter()
            .enumerate()
            .map(|(i, id)| (id, i))
            .collect();
...
mod tests {
    use super::*;
    use crate::gate_c::types::{Operation, DataRef};
    ...
    fn create_test_step(id: &str, inputs: Vec<&str>, outputs: Vec<&str>) -> PipelineStep {
        PipelineStep {
            ...
            inputs: inputs.into_iter().map(|s| DataRef {
                id: s.to_string(),
                data_type: "test".to_string(),
                source_step: None,
            }).collect(),
            outputs: outputs.into_iter().map(|s| DataRef {
                id: s.to_string(),
                data_type: "test".to_string(),
    ...
    fn test_pipeline_too_large() {
        ...
        match result.unwrap_err() {
            crate::gate_c::error::GateCError::Pipeline(PipelineError::PipelineTooLarge { steps, limit }) => {
                assert_eq!(steps, 3);
                assert_eq!(limit, 2);
            }
