...
fn test_canonical_plan_snapshot_determinism() {
    ...
    for (i, bytes) in canonical_bytes.iter().enumerate() {
        assert_eq!(
            bytes, first_bytes,
            "Canonical bytes not deterministic at run {}",
            i
        );
    }
...
fn test_complex_plan_snapshot_determinism() {
    ...
    let canonical1 = normalizer
        .normalize(&plan)
        .expect("Complex normalization should succeed");
    let canonical2 = normalizer
        .normalize(&plan)
        .expect("Complex normalization should succeed");
...
fn test_repl_visualization_snapshot_determinism() {
    ...
    assert_eq!(
        viz1.metadata.render_duration_ms,
        viz2.metadata.render_duration_ms
    );
...
fn test_pipeline_planning_snapshot_determinism() {
    ...
    let pipeline_steps: Vec<PipelineStep> = plan
        .steps
        .into_iter()
    ...
    let pipeline1 = planner
        .plan(pipeline_steps.clone())
        .expect("Pipeline planning should succeed");
    let pipeline2 = planner
        .plan(pipeline_steps)
        .expect("Pipeline planning should succeed");
