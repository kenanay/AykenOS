...
use crate::gate_c::{
    error::{GateCResult, SecurityError},
    limits::{MAX_INSPECT_OUTPUT_BYTES, MAX_PLAN_STEPS},
    types::{DataRef, ExecutionPlan, MutationIntent, Operation, PlanStep},
};
...
impl SecurityInspector {
    ...
    pub fn inspect_plan(&self, plan: &ExecutionPlan) -> GateCResult<SecurityReport> {
        ...
        let start_time =
            crate::gate_c::deterministic::deterministic_timestamp_from_plan_id(&plan.id);

        // DETERMINISM FIX: Use deterministic duration based on plan content
        let inspection_duration_ms = crate::gate_c::deterministic::deterministic_duration_ms(
            "security_inspection",
            &plan.id,
        );
    ...
    fn analyze_plan_structure(
        ...
        if plan.steps.is_empty() {
            findings.push(SecurityFinding {
                ...
                description:
                    "Plan contains no steps, which may indicate incomplete or malicious plan"
                        .to_string(),
                step_id: None,
        ...
        if plan.steps.len() > MAX_PLAN_STEPS / 2 {
            findings.push(SecurityFinding {
                ...
                category: SecurityCategory::Structure,
                description: format!(
                    "Plan has {} steps, approaching complexity limits",
                    plan.steps.len()
                ),
                step_id: None,
    ...
    fn analyze_step(
        ...
        step: &PlanStep,
        findings: &mut Vec<SecurityFinding>,
        capability_requirements: &mut Vec<CapabilityRequirement>,
    ) -> GateCResult<()> {
        // Analyze operation type
        match &step.operation {
            Operation::Query { target, parameters } => {
                self.analyze_query_operation(
                    step,
                    target,
                    parameters,
                    findings,
            ...
            Operation::Compute {
                ...
                self.analyze_compute_operation(
                    step,
                    function,
                    arguments,
                    findings,
        ...
        for (key, value) in parameters {
            if value.contains("'") || value.contains("--") || value.contains(";") {
                findings.push(SecurityFinding {
                    ...
                    category: SecurityCategory::DataAccess,
                    description: format!(
                        "Parameter '{}' contains potential injection patterns",
                        key
                    ),
                    step_id: Some(step.id.clone()),
    ...
    fn analyze_compute_operation(
        ...
        if arguments.len() > 20 {
            findings.push(SecurityFinding {
                ...
                category: SecurityCategory::Resource,
                description: format!(
                    "Function has {} arguments, may indicate complexity issues",
                    arguments.len()
                ),
                step_id: Some(step.id.clone()),
    ...
    fn analyze_data_references(
        ...
        if total_refs > 10 {
            findings.push(SecurityFinding {
                ...
                category: SecurityCategory::DataAccess,
                description: format!(
                    "Step has {} data references, may indicate design issues",
                    total_refs
                ),
                step_id: Some(step.id.clone()),
        ...
        for data_ref in step.inputs.iter().chain(step.outputs.iter()) {
            if data_ref.id.contains("password") || data_ref.id.contains("secret") {
                findings.push(SecurityFinding {
                    ...
                    category: SecurityCategory::DataAccess,
                    description: format!(
                        "Data reference may contain sensitive data: {}",
                        data_ref.id
                    ),
                    step_id: Some(step.id.clone()),
                    recommendations: vec![
                        "Ensure proper access controls for sensitive data".to_string()
                    ],
                });
            }
        }
        ...
        for (data_id, consumers) in &data_consumers {
            if consumers.len() > 5 {
                findings.push(SecurityFinding {
                    ...
                    category: SecurityCategory::DataAccess,
                    description: format!(
                        "Data '{}' is consumed by {} steps, may indicate over-sharing",
                        data_id,
                        consumers.len()
                    ),
                    step_id: None,
    ...
    fn analyze_mutations(
        ...
        plan: &ExecutionPlan,
        findings: &mut Vec<SecurityFinding>,
        _capability_requirements: &mut Vec<CapabilityRequirement>,
    ) -> GateCResult<()> {
        ...
        if mutation_count > 10 {
            findings.push(SecurityFinding {
                ...
                category: SecurityCategory::Mutation,
                description: format!(
                    "Plan contains {} mutation operations, may indicate high impact",
                    mutation_count
                ),
                step_id: None,
            ...
            for (pattern, replacement) in &default_patterns {
                if finding.description.to_lowercase().contains(pattern) {
                    ...
                    finding.description = finding
                        .description
                        .replace(&pattern.to_uppercase(), replacement);
                    finding.description = finding
                        .description
                        .replace(&pattern.to_lowercase(), replacement);
                }
                ...
                for recommendation in &mut finding.recommendations {
                    if recommendation.to_lowercase().contains(pattern) {
                        ...
                        *recommendation =
                            recommendation.replace(&pattern.to_uppercase(), replacement);
                        *recommendation =
                            recommendation.replace(&pattern.to_lowercase(), replacement);
                    }
                }
            }
        }
        ...
        for risk_factor in &mut report.risk_assessment.risk_factors {
            for (pattern, replacement) in &default_patterns {
                ...
                if risk_factor.description.to_lowercase().contains(pattern) {
                    ...
                    risk_factor.description = risk_factor
                        .description
                        .replace(&pattern.to_uppercase(), replacement);
                    risk_factor.description = risk_factor
                        .description
...
impl RedactionEngine {
    ...
    pub fn redact_sensitive(&self, report: &SecurityReport) -> GateCResult<RedactedSecurityReport> {
        ...
        // Redact plan ID
        let redacted_plan_id =
            self.redact_text(&report.plan_id, &mut redaction_summary.items_redacted);
        ...
        let redacted_risk_assessment = self.redact_risk_assessment(
            &report.risk_assessment,
            &mut redaction_summary.items_redacted,
        )?;

        // Filter capability requirements
        let filtered_capabilities = self.filter_capabilities(
            &report.capability_requirements,
            &mut redaction_summary.capabilities_filtered,
        )?;

        // Redact audit metadata
        let redacted_audit_metadata = self.redact_audit_metadata(
    ...
    fn redact_findings(
        ...
        redaction_summary: &mut RedactionSummary,
    ) -> GateCResult<Vec<RedactedSecurityFinding>> {
        ...
        for finding in findings {
            ...
            let redacted_finding = RedactedSecurityFinding {
                ...
                description: self
                    .redact_text(&finding.description, &mut redaction_summary.items_redacted),
                step_id: finding
                    .step_id
                    .as_ref()
                    .map(|id| self.redact_text(id, &mut redaction_summary.items_redacted)),
    ...
    /// Redact risk assessment
    fn redact_risk_assessment(
        &self,
        risk_assessment: &RiskAssessment,
        redaction_count: &mut usize,
    ) -> GateCResult<RedactedRiskAssessment> {
    ...
    ) -> GateCResult<Vec<CapabilityRequirement>> {
    ...
    /// Redact audit metadata
    fn redact_audit_metadata(
        &self,
        metadata: &AuditMetadata,
        redaction_count: &mut usize,
    ) -> GateCResult<RedactedAuditMetadata> {
    ...
    fn estimate_report_size(&self, report: &SecurityReport) -> GateCResult<usize> {
        ...
        let serialized = serde_json::to_string(report).map_err(|e| {
            SecurityError::RedactionFailed(format!("Failed to serialize report: {}", e))
        })?;
        Ok(serialized.len())
    }
    ...
    fn estimate_redacted_report_size(&self, report: &RedactedSecurityReport) -> GateCResult<usize> {
        let serialized = serde_json::to_string(report).map_err(|e| {
            SecurityError::RedactionFailed(format!("Failed to serialize redacted report: {}", e))
        })?;
        Ok(serialized.len())
    }
    ...
    fn apply_size_limits(
        &self,
        mut report: RedactedSecurityReport,
    ) -> GateCResult<RedactedSecurityReport> {
        ...
        if report.findings.len() > 10 {
            ...
            if summarized_findings.len() < report.findings.len() {
                summarized_findings.push(RedactedSecurityFinding {
                    ...
                    category: SecurityCategory::Structure,
                    description: format!(
                        "Report truncated: showing {} of {} findings",
                        summarized_findings.len(),
                        report.findings.len()
                    ),
                    step_id: None,
...
mod tests {
    ...
    use crate::gate_c::types::{
        ContentSpec, InvalidationReason, MutationIntent, Operation, PlanMetadata, ResourcePath,
    };
    ...
    fn test_empty_plan_detection() {
        ...
        let report = result.unwrap();
        let empty_plan_finding = report.findings.iter().find(|f| f.id == "EMPTY_PLAN");
    ...
    fn test_sensitive_data_detection() {
        ...
        let report = result.unwrap();
        let sensitive_findings: Vec<_> = report
            .findings
            .iter()
    ...
    fn test_sensitive_data_redaction() {
        ...
        let sensitive_findings: Vec<_> = redacted_report
            .findings
            .iter()
    ...
    fn test_large_report_summarization() {
        ...
        if redacted_report.findings.len() < report.findings.len() {
            let summary_finding = redacted_report
                .findings
                .iter()
    ...
    fn test_capability_bypass_attempt_admin_escalation() {
        ...
        let admin_capabilities: Vec<_> = redacted_report
            .capability_requirements
            .iter()
        ...
        assert!(
            ...
            "Admin capabilities should be filtered out"
        );
    ...
    fn test_capability_bypass_attempt_scope_elevation() {
        ...
        let execute_capabilities: Vec<_> = redacted_report
            .capability_requirements
            .iter()
            .filter(|c| c.scope == CapabilityScope::Execute)
            .collect();
        assert!(
            execute_capabilities.is_empty(),
            "Execute capabilities should be filtered out with write-only scope"
        );
    ...
    fn test_capability_bypass_attempt_malicious_patterns() {
        ...
        assert!(
            ...
            "Should detect sensitive patterns in various forms"
        );
    ...
    fn test_dos_resistance_redaction_performance() {
        ...
        // Should complete within reasonable time
        assert!(
            inspection_duration.as_secs() < 10,
            "Inspection should complete within 10 seconds"
        );
        assert!(
            redaction_duration.as_secs() < 5,
            "Redaction should complete within 5 seconds"
        );
        ...
        assert!(redacted_report.redaction_summary.redacted_size <= MAX_INSPECT_OUTPUT_BYTES * 2);
        // Allow some overhead
    }
    ...
    fn test_performance_security_operations_baseline() {
        ...
        assert!(
            inspection_duration.as_millis() < 100,
            "Baseline inspection should be under 100ms"
        );
        assert!(
            redaction_duration.as_millis() < 50,
            "Baseline redaction should be under 50ms"
        );
    ...
    fn test_performance_security_operations_scaling() {
        ...
        for (size, inspection_duration, redaction_duration) in performance_data {
            ...
            assert!(
                inspection_duration.as_millis() <= max_inspection_ms,
                "Inspection performance should scale linearly (size: {}, duration: {}ms)",
                size,
                inspection_duration.as_millis()
            );
            assert!(
                redaction_duration.as_millis() <= max_redaction_ms,
                "Redaction performance should scale linearly (size: {}, duration: {}ms)",
                size,
                redaction_duration.as_millis()
            );
        }
    }
    ...
    fn test_performance_security_operations_memory_usage() {
        ...
        assert!(
            redacted_result.is_ok(),
            "Should handle memory-intensive redaction"
        );

        // Should detect excessive data references
        let excessive_refs_finding = report
            .findings
            .iter()
            .find(|f| f.id == "EXCESSIVE_DATA_REFS");
        assert!(
            excessive_refs_finding.is_some(),
            "Should detect excessive data references"
        );
    }
}
