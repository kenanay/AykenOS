...
use crate::bcib::{BCIBInstruction, ContextInstruction, OperandRef, QueryInstruction};
...
impl DependencyTracker {
    ...
    fn analyze_instruction_registers(
        &self,
        instruction: &BCIBInstruction,
    ) -> Result<(Vec<RegisterId>, Vec<RegisterId>), DependencyError> {
        match instruction {
            ...
            _ => Err(DependencyError::InvalidInstruction {
        ...
        if let Some(neighbors) = adj_list.get(node) {
            for neighbor in neighbors {
                if !visited.contains(neighbor) {
                    if let Some(cycle) =
                        self.dfs_cycle_detection(neighbor, adj_list, visited, rec_stack)?
                    {
...
mod tests {
    ...
    fn test_simple_dependency_analysis() {
        ...
        assert_eq!(
            graph.register_definitions.get(&RegisterId::Context(0)),
            Some(&InstructionId(0))
        );
        assert_eq!(
            graph.register_definitions.get(&RegisterId::Data(1)),
            Some(&InstructionId(1))
        );
    ...
    fn test_use_before_define_detection() {
        ...
        let instructions = vec![NormalizedInstruction {
            instruction: BCIBInstruction::Context(ContextInstruction::Return {
                location: test_location(),
            }),
    ...
    fn test_no_cycles_valid_graph() {
        ...
        // Check register definitions
        assert_eq!(
            graph.register_definitions.get(&RegisterId::Data(0)),
            Some(&InstructionId(0))
        );
        assert_eq!(
            graph.register_definitions.get(&RegisterId::Data(1)),
