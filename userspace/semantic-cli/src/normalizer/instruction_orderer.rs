...
use crate::bcib::{BCIBInstruction, BCIBSequence, ContextInstruction, QueryInstruction};
use crate::normalizer::dependency_tracker::{DependencyGraph, InstructionId};
use crate::normalizer::{InstructionGroup, NormalizedInstruction};
...
impl InstructionOrderer {
    ...
    ) -> Result<Vec<NormalizedInstruction>, OrderingError> {
    ...
    fn group_instructions_by_type<'a>(
        &self,
        bcib: &'a BCIBSequence,
    ) -> Result<HashMap<InstructionGroup, Vec<(usize, &'a BCIBInstruction)>>, OrderingError> {
        let mut groups: HashMap<InstructionGroup, Vec<(usize, &'a BCIBInstruction)>> =
            HashMap::new();
    ...
    fn order_within_group(
        &self,
        group_instructions: &[(usize, &BCIBInstruction)],
        _dependencies: &DependencyGraph,
    ) -> Result<Vec<NormalizedInstruction>, OrderingError> {
    ...
    fn create_normalized_instruction(
        &self,
        idx: usize,
        instruction: &BCIBInstruction,
        dependencies: &DependencyGraph,
    ) -> Result<NormalizedInstruction, OrderingError> {
        let instruction_id = InstructionId(idx);

        // Find dependency node for this instruction
        let dep_node = dependencies
            .nodes
            .iter()
    ...
    pub fn apply_topological_order(
        &self,
    ...
    fn compute_topological_order(
        &self,
        dependencies: &DependencyGraph,
    ) -> Result<Vec<InstructionId>, OrderingError> {
        ...
        for node in &dependencies.nodes {
            for dep in &node.dependencies {
                adj_list
                    .get_mut(dep)
                    .unwrap()
                    .push(node.instruction_id.clone());
    ...
    fn validate_dependency_order(
        &self,
        ordered_instructions: &[NormalizedInstruction],
        dependencies: &DependencyGraph,
        ...
        for (orig_idx, dep_node) in dependencies.nodes.iter().enumerate() {
            ...
            for dep_instruction in &dep_node.dependencies {
                let dep_pos = position_map.get(&dep_instruction.0).ok_or_else(|| {
                    OrderingError::DependencyViolation {
                        constraint: format!(
                            "Dependency instruction {} not found",
                            dep_instruction.0
                        ),
                    }
                })?;
...
mod tests {
    use super::*;
    use crate::bcib::{BCIBMetadata, ComparisonOp, OperandRef, Value};
    ...
    fn test_instruction_classification() {
        ...
        assert_eq!(
            orderer.classify_instruction(&load_context).unwrap(),
            InstructionGroup::Context
        );
        ...
        assert_eq!(
            orderer.classify_instruction(&load_field).unwrap(),
            InstructionGroup::Data
        );
        ...
        assert_eq!(
            orderer.classify_instruction(&compare).unwrap(),
            InstructionGroup::Compute
        );
        ...
        assert_eq!(
            orderer.classify_instruction(&return_inst).unwrap(),
            InstructionGroup::Control
        );
    }
