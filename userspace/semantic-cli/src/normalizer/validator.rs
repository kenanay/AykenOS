...
use crate::bcib::{BCIBSequence, BCIBInstruction, ContextInstruction, QueryInstruction, OperandRef};
...
impl NormalizationValidator {
    ...
    /// 
    /// **Architectural Reference:** C1 Section "Input Validation"
    fn validate_instruction_structure(&self, instruction: &BCIBInstruction, idx: usize) -> Result<(), ValidationError> {
        match instruction {
            ...
            _ => {
                Err(ValidationError::InvalidInstruction {
                    instruction: format!("Unknown instruction type at index {}: {:?}", idx, instruction),
                })
            }
        }
    }
    ...
    fn validate_register_references(&self, bcib: &BCIBSequence) -> Result<(), ValidationError> {
        ...
        for instruction in &bcib.instructions {
            match instruction {
                ...
                BCIBInstruction::Query(QueryInstruction::Compare { target_register, left, right, .. }) => {
                    defined_registers.insert(RegisterId::Data(*target_register));
                    // Check operand usage
                ...
                BCIBInstruction::Query(QueryInstruction::LogicalOp { target_register, operands, .. }) => {
                    defined_registers.insert(RegisterId::Data(*target_register));
    ...
    fn validate_context_references(&self, bcib: &BCIBSequence) -> Result<(), ValidationError> {
        for instruction in &bcib.instructions {
            if let BCIBInstruction::Context(ContextInstruction::LoadContext { path, .. }) = instruction {
                ...
                if !path.chars().all(|c: char| c.is_alphanumeric() || c == '_' || c == '.') {
                    return Err(ValidationError::InvalidContextReference {
                        context: path.clone(),
                    });
                }
    ...
    fn validate_instruction_coherence(&self, bcib: &BCIBSequence) -> Result<(), ValidationError> {
        ...
        for instruction in &bcib.instructions {
            if matches!(instruction, BCIBInstruction::Context(ContextInstruction::Return { .. })) {
                has_return = true;
            }
        }
    ...
    /// 
    /// **Architectural Reference:** C1 Section "Output Validation"
    fn validate_all_filters_normalized(&self, normalized: &NormalizedBCIB) -> Result<(), ValidationError> {
    ...
    /// 
    /// **Architectural Reference:** C1 Section "Output Validation"
    fn validate_register_allocation_complete(&self, normalized: &NormalizedBCIB) -> Result<(), ValidationError> {
    ...
    /// 
    /// **Architectural Reference:** C1 Section "Output Validation"
    fn validate_no_unreachable_instructions(&self, normalized: &NormalizedBCIB) -> Result<(), ValidationError> {
    ...
    /// 
    /// **Architectural Reference:** C1 Section "Output Validation"
    fn validate_instruction_metadata_consistency(&self, normalized: &NormalizedBCIB) -> Result<(), ValidationError> {
...
mod tests {
    ...
    use crate::normalizer::{NormalizedInstruction, InstructionGroup, NormalizedMetadata, RegisterAllocation};
