...
use indexed_register_allocator::{IndexedRegisterAllocator, IndexedRegisterAllocation};
use dependency_tracker::{DependencyTracker, RegisterId};
use streaming_dependency_builder::{StreamingDependencyBuilder, IndexedDependencyGraph};
...
impl OptimizedBCIBNormalizer {
    ...
    pub fn normalize(&mut self, bcib: BCIBSequence) -> Result<NormalizedBCIB, NormalizationError> {
        ...
        let indexed_allocation = self.indexed_register_allocator.allocate_for_sequence(&bcib)
            .map_err(|e| NormalizationError::RegisterAllocationFailed { 
                reason: e.to_string() 
            })?;
    ...
    pub fn get_performance_stats(&self) -> indexed_register_allocator::IndexedRegisterAllocationStats {
...
impl CompleteSinglePassBCIBNormalizer {
    ...
    pub fn normalize(&mut self, bcib: BCIBSequence) -> Result<NormalizedBCIB, NormalizationError> {
        self.complete_normalizer.normalize_complete_single_pass(bcib)
    }
    
    /// Get comprehensive performance statistics
    pub fn get_performance_stats(&self) -> complete_single_pass_normalizer::CompleteSinglePassStats {
...
pub fn normalize_bcib_complete_single_pass(input: BCIBSequence) -> Result<NormalizedBCIB, NormalizationError> {
...
mod tests {
    ...
    fn test_optimized_normalizer_performance() {
        ...
        assert_eq!(normalized_original.instructions.len(), normalized_optimized.instructions.len());
        assert_eq!(normalized_original.register_allocation.allocated_registers.len(), 
                   normalized_optimized.register_allocation.allocated_registers.len());
        ...
        println!("Original duration: {:?}, Optimized duration: {:?}", duration_original, duration_optimized);
        
        // The optimized version should not be significantly slower
        assert!(duration_optimized <= duration_original * 2, 
    ...
    fn test_complete_single_pass_normalizer_performance() {
        ...
        let mut streaming_normalizer = StreamingOptimizedBCIBNormalizer::with_capacity(bcib.instructions.len());
        ...
        // Results should be functionally equivalent
        assert_eq!(normalized_original.instructions.len(), normalized_optimized.instructions.len());
        assert_eq!(normalized_original.instructions.len(), normalized_streaming.instructions.len());
        assert_eq!(normalized_original.instructions.len(), normalized_complete.instructions.len());
        
        // Performance comparison
        println!("Original (7-pass) duration: {:?}", duration_original);
        println!("Optimized (multi-pass + indexed) duration: {:?}", duration_optimized);
        println!("Streaming (single-pass) duration: {:?}", duration_streaming);
        println!("Complete Single-Pass duration: {:?}", duration_complete);
        
        // Calculate performance ratios
        let complete_vs_original_ratio = duration_complete.as_nanos() as f64 / duration_original.as_nanos() as f64;
        println!("Complete Single-Pass vs Original ratio: {:.2}x", 1.0 / complete_vs_original_ratio);
        
        // Complete single-pass should be fastest or at least competitive
        assert!(duration_complete <= duration_original * 2, 
    ...
    fn test_complete_single_pass_normalizer_foundation() {
        ...
        let mut complete_normalizer = CompleteSinglePassBCIBNormalizer::with_capacity(bcib.instructions.len());
        ...
        assert!(!normalized.register_allocation.allocated_registers.is_empty());
        ...
        let context_instructions: Vec<_> = normalized.instructions.iter()
            .filter(|inst| matches!(inst.instruction_group, InstructionGroup::Context))
            .collect();
        let data_instructions: Vec<_> = normalized.instructions.iter()
            .filter(|inst| matches!(inst.instruction_group, InstructionGroup::Data))
            .collect();
        let compute_instructions: Vec<_> = normalized.instructions.iter()
        ...
        let groups: Vec<_> = normalized.instructions.iter()
        ...
        println!("   - Single-pass processing: {} instructions", stats.instructions_processed);
        println!("   - Total operations: {}", stats.total_operations);
        println!("   - Memory allocations: {}", stats.memory_allocations);
        println!("   - Canonical groups used: {}", stats.canonical_groups_used);
    ...
    fn test_complete_single_pass_vs_multi_pass_performance() {
        ...
        if result_single.is_ok() {
            ...
            // Results should be functionally equivalent
            assert_eq!(normalized_multi.instructions.len(), normalized_single.instructions.len());
            
            // Performance comparison
            println!("Multi-pass duration: {:?}", duration_multi);
            println!("Complete single-pass duration: {:?}", duration_single);
            
            let performance_ratio = duration_single.as_nanos() as f64 / duration_multi.as_nanos() as f64;
            println!("Single-pass vs Multi-pass ratio: {:.2}x", 1.0 / performance_ratio);
            
            // Single-pass should be competitive or better
            assert!(duration_single <= duration_multi * 3, 
            ...
            println!("Complete single-pass normalizer needs further refinement for complex sequences");
        }
    }
    ...
    fn test_streaming_normalizer_performance() {
        ...
        let streaming_vs_original_ratio = duration_streaming.as_nanos() as f64 / duration_original.as_nanos() as f64;
        println!("Streaming vs Original ratio: {:.2}x", 1.0 / streaming_vs_original_ratio);
        ...
        assert!(duration_streaming <= duration_original * 3, 
    ...
    fn test_streaming_normalizer_foundation() {
        ...
        println!("   - Canonical ordering: Context → Data → Compute → Control");
        println!("   - Performance tracking: {} processed instructions", stats.instructions_processed);
    }
}