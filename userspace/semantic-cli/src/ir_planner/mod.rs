...
use crate::execution_plan::{BlockId, BlockTerminator, ExecutionPlan, IRInstruction, RegisterId};
...
use crate::parallelism::{
    AdaptiveDecisionEngine, DefaultDecisionEngine, ParallelExecutor, RayonParallelExecutor,
};
...
impl IRExecutor {
    ...
    fn execute_sequential_path(
        ...
        while !self.execution_state.is_terminated {
            ...
            self.replay_recorder
                .record_step(&self.execution_state, &self.register_file)?;
    ...
    pub fn execute_with_replay(
        &mut self,
        plan: ExecutionPlan,
    ) -> Result<(ExecutionResult, ReplayTrace), ExecutionError> {
        ...
        self.replay_recorder
            .set_context_snapshots(context_snapshots);
    ...
    fn initialize_execution(&mut self, plan: &ExecutionPlan) -> Result<(), ExecutionError> {
        ...
        plan.validate()
            .map_err(|e| ExecutionError::InvalidExecutionPlan {
                reason: e.to_string(),
            })?;
    ...
    fn fetch_next_instruction<'a>(
        &self,
        plan: &'a ExecutionPlan,
    ) -> Result<&'a IRInstruction, ExecutionError> {
        let current_block = plan.get_block(self.execution_state.current_block).ok_or(
        ...
        current_block.instructions.get(instruction_index).ok_or(
            ExecutionError::InstructionIndexOutOfBounds {
                block_id: self.execution_state.current_block,
    ...
    fn execute_load_context(
        ...
        self.register_file
            .set_register(target_register, register_value)?;
    ...
    fn execute_load_field(
                ...
                if context_data.items().len() == 1 {
                    ...
                    match field_val {
                        ...
                        serde_json::Value::Number(n) => {
                            RegisterValue::Number(n.as_f64().unwrap_or(0.0))
                        }
                        serde_json::Value::Bool(b) => RegisterValue::Boolean(b),
                        serde_json::Value::Null => RegisterValue::Null,
        ...
        self.register_file
            .set_register(target_register, field_value)?;
    ...
    fn execute_compare(
        ...
        self.register_file
            .set_register(target_register, RegisterValue::Boolean(result))?;
    ...
    fn execute_logical_op(
        ...
        operand_registers: &[RegisterId],
        target_register: RegisterId,
    ) -> Result<(), ExecutionError> {
        let operand_values: Result<Vec<_>, _> = operand_registers
            .iter()
    ...
    fn evaluate_filter_expression(
        ...
        let current_item = self.execution_state.current_item.as_ref().ok_or_else(|| {
            ...
            OperandRef::Literal(lit) => {
                ...
                match lit {
                    ...
                    Value::Array(_) | Value::List(_) | Value::SortedMap(_) => {
                        return Err(ExecutionError::InvalidOperation {
                            operation: "Collection values cannot be used in filter expressions"
                                .to_string(),
                        });
                    }
                }
            }
            OperandRef::Field(field_name) => current_item
                .get(field_name)
                .cloned()
                .unwrap_or(serde_json::Value::Null),
    ...
    fn compare_json_values(
        ...
        match (left, right) {
            (serde_json::Value::String(l), serde_json::Value::String(r)) => Ok(match operator {
            ...
            (serde_json::Value::Bool(l), serde_json::Value::Bool(r)) => Ok(match operator {
                ComparisonOp::Equal => l == r,
                ComparisonOp::NotEqual => l != r,
                _ => {
                    return Err(ExecutionError::InvalidOperation {
    ...
    fn perform_comparison(
        ...
        match (left, right) {
            (RegisterValue::String(l), RegisterValue::String(r)) => Ok(match operator {
                ComparisonOp::Equal => l == r,
                ComparisonOp::NotEqual => l != r,
                ComparisonOp::LessThan => l < r,
                ComparisonOp::LessThanOrEqual => l <= r,
            ...
            (RegisterValue::Number(l), RegisterValue::Number(r)) => Ok(match operator {
            ...
            (RegisterValue::Boolean(l), RegisterValue::Boolean(r)) => Ok(match operator {
                ComparisonOp::Equal => l == r,
                ComparisonOp::NotEqual => l != r,
                _ => {
                    return Err(ExecutionError::InvalidOperation {
    ...
    fn perform_logical_operation(
        &self,
        operation: LogicalOperator,
        operands: &[&RegisterValue],
    ) -> Result<bool, ExecutionError> {
        match operation {
            LogicalOperator::And => {
                if operands.len() != 2 {
    ...
    fn finalize_execution(&self) -> Result<ExecutionResult, ExecutionError> {
        match &self.execution_state.termination_reason {
            ...
            Some(TerminationReason::Error { error }) => Err(error.clone()),
            Some(TerminationReason::Timeout) => Err(ExecutionError::ExecutionTimeout {
                steps: self.execution_state.execution_step,
            }),
            None => Err(ExecutionError::UnexpectedTermination),
        }
    }
    ...
    fn capture_context_snapshots(
        &self,
        _plan: &ExecutionPlan,
    ) -> Result<HashMap<String, crate::context::ContextData>, ExecutionError> {
...
impl From<register_file::RegisterFileError> for ExecutionError {
    fn from(error: register_file::RegisterFileError) -> Self {
        ExecutionError::RegisterFileError {
...
mod tests {
    ...
    use crate::execution_plan::{ExecutionMetadata, ExecutionPlan, IRBlock, ParallelSafety};
    ...
    fn test_comparison_operations() {
        ...
        executor
            .register_file
            .set_register(0, RegisterValue::Number(10.0))
            .unwrap();
        executor
            .register_file
            .set_register(1, RegisterValue::Number(20.0))
            .unwrap();
    ...
    fn test_replay_mode_prevents_parallel_execution() {
        ...
        executor
            .replay_recorder
            .initialize("test_fingerprint".to_string());
