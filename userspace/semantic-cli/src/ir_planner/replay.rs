...
impl ReplayTrace {
    ...
    pub fn validate(&self) -> Result<(), ReplayError> {
        ...
        for (i, step) in self.steps.iter().enumerate() {
            if step.step_number != (i as u64) {
                return Err(ReplayError::InvalidTrace {
                    reason: format!(
                        "Step sequence mismatch at index {}: expected {}, got {}",
    ...
    pub fn to_json(&self) -> Result<String, ReplayError> {
        serde_json::to_string_pretty(self).map_err(|e| ReplayError::SerializationFailed {
            reason: e.to_string(),
        })
    }
    ...
    pub fn from_json(json: &str) -> Result<Self, ReplayError> {
        let trace: ReplayTrace =
            serde_json::from_str(json).map_err(|e| ReplayError::SerializationFailed {
                reason: e.to_string(),
            })?;
...
impl ReplayStep {
    ...
    pub fn new(
        step_number: u64,
        execution_state: ExecutionState,
        register_state: HashMap<RegisterId, RegisterValue>,
    ) -> Self {
        Self {
            step_number,
            execution_state,
