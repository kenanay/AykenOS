...
impl DeterministicPartitioner {
    ...
    pub fn partition_iterations(
        ...
        let partitions = self.create_fixed_chunk_partitions(
            total_iterations,
            chunk_size,
    ...
    pub fn calculate_deterministic_chunk_size(&self, total_iterations: u32) -> u32 {
        if total_iterations <= self.config.min_chunk_size {
            ...
            (total_iterations + self.config.target_chunks - 1) / self.config.target_chunks // Ceiling division
        } else {
    ...
    fn create_fixed_chunk_partitions(
        ...
        if all_partitions.len() <= effective_partitions {
            ...
            let partitions_per_worker = (all_partitions.len() + effective_partitions - 1) / effective_partitions;
            ...
            for worker_id in 0..effective_partitions {
                ...
                if start_idx < all_partitions.len() {
                    ...
                    let merged_partition = IterationPartition {
                        ...
                        iteration_count: last_partition.end_iteration - first_partition.start_iteration,
                    };
...
impl DeterministicPartitioner {
    ...
    pub fn analyze_partitioning(
        ...
        let balance_ratio = if max_size > 0 { min_size as f64 / max_size as f64 } else { 1.0 };
...
mod tests {
    ...
    fn test_determinism_across_different_parallelism() {
        ...
        for partition in &partitions1 {
            for i in partition.start_iteration..partition.end_iteration {
                assert!(!covered1[i as usize], "Iteration {} covered twice in partitions1", i);
                covered1[i as usize] = true;
            }
        }
        assert!(covered1.iter().all(|&x| x), "Not all iterations covered in partitions1");
        
        let mut covered2 = vec![false; 1000];
        for partition in &partitions2 {
            for i in partition.start_iteration..partition.end_iteration {
                assert!(!covered2[i as usize], "Iteration {} covered twice in partitions2", i);
                covered2[i as usize] = true;
            }
        }
        assert!(covered2.iter().all(|&x| x), "Not all iterations covered in partitions2");
        
        // Verify that partitions are contiguous and non-overlapping
        for partitions in [&partitions1, &partitions2] {
            let mut last_end = 0;
            for partition in partitions {
                assert_eq!(partition.start_iteration, last_end, "Gap in partition coverage");
    ...
    fn test_partition_verification() {
        ...
        let partitions = partitioner.partition_iterations(1000, 4).unwrap();
        assert!(partitioner.verify_partition_correctness(&partitions, 1000).is_ok());
    ...
    fn test_property_determinism() {
        ...
        for total_iterations in [100, 500, 1000, 5000] {
            for available_parallelism in [1, 2, 4, 8] {
                let partitions1 = partitioner.partition_iterations(total_iterations, available_parallelism).unwrap();
                let partitions2 = partitioner.partition_iterations(total_iterations, available_parallelism).unwrap();
                let partitions3 = partitioner.partition_iterations(total_iterations, available_parallelism).unwrap();
    ...
    fn test_property_completeness() {
        ...
        for total_iterations in [1, 10, 100, 1000, 10000] {
            let partitions = partitioner.partition_iterations(total_iterations, 4).unwrap();
