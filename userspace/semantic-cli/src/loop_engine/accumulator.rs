...
impl AccumulatorPattern {
    ...
    pub fn update_accumulator(&mut self, name: &str, new_value: Value) -> Result<()> {
        let accumulator = self.accumulators.get_mut(name).ok_or_else(|| {
    ...
    pub fn get_accumulator(&self, name: &str) -> Result<&Value> {
        let accumulator = self.accumulators.get(name).ok_or_else(|| {
            SemanticCLIError::validation_error(
                format!("Accumulator '{}' not found", name),
...
mod tests {
    ...
    fn test_single_accumulator() {
        let mut acc = LoopAccumulator::new("counter".to_string(), Value::Number(0.0));
    ...
    fn test_accumulator_validation() {
        let acc = LoopAccumulator::new("test".to_string(), Value::Boolean(true));
        assert!(acc.validate().is_ok());

        // Test empty name
        let invalid_acc = LoopAccumulator::new("".to_string(), Value::Boolean(true));
    ...
    fn test_multi_accumulator_pattern() {
        ...
        assert!(pattern
            ...
            .is_ok());
        assert!(pattern
            .add_accumulator("message".to_string(), Value::String("".to_string()))
            .is_ok());
        ...
        // Test updates
        assert!(pattern
            .update_accumulator("counter", Value::Number(10.0))
            .is_ok());
        assert!(pattern
            .update_accumulator("flag", Value::Boolean(true))
            .is_ok());
        assert!(pattern
            .update_accumulator("message", Value::String("hello".to_string()))
            .is_ok());

        // Test type validation
        assert!(pattern
            .update_accumulator("counter", Value::String("invalid".to_string()))
            .is_err());

        // Test retrieval
        assert_eq!(
            pattern.get_accumulator("counter").unwrap(),
            &Value::Number(10.0)
        );
        assert_eq!(
            pattern.get_accumulator("flag").unwrap(),
            &Value::Boolean(true)
        );
        assert_eq!(
            pattern.get_accumulator("message").unwrap(),
            &Value::String("hello".to_string())
        );

        // Test expected types
        assert_eq!(
            pattern.get_expected_type("counter").unwrap(),
            ValueType::Number
        );
        assert_eq!(
            pattern.get_expected_type("flag").unwrap(),
            ValueType::Boolean
        );
        assert_eq!(
            pattern.get_expected_type("message").unwrap(),
            ValueType::String
        );
    }
    ...
    fn test_accumulator_pattern_errors() {
        ...
        assert!(pattern
            .add_accumulator("test".to_string(), Value::Number(0.0))
            .is_ok());
        assert!(pattern
            .add_accumulator("test".to_string(), Value::Number(1.0))
            .is_err());

        // Test non-existent accumulator
        assert!(pattern.get_accumulator("nonexistent").is_err());
        assert!(pattern
            .update_accumulator("nonexistent", Value::Number(0.0))
            .is_err());
    ...
    fn test_accumulator_pattern_from_initial_values() {
        ...
        assert_eq!(
            pattern.get_accumulator("c").unwrap(),
    ...
    fn test_get_all_values() {
        let mut pattern = AccumulatorPattern::new();
        pattern
            .add_accumulator("x".to_string(), Value::Number(10.0))
            .unwrap();
        pattern
            .add_accumulator("y".to_string(), Value::Boolean(false))
            .unwrap();
