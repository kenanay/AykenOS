...
fn test_p1_determinism_over_parallelism() {
    ...
    let verdict = checker
        .verify_execution_request(&unsafe_block, ExecutionMode::Normal, 1000)
        .unwrap();
...
fn test_p2_ir_single_source_of_truth() {
    ...
    let verdict = checker
        .verify_execution_request(&safe_block, ExecutionMode::Normal, 1000)
        .unwrap();
    ...
    let verdict = checker
        .verify_execution_request(&reduction_block, ExecutionMode::Normal, 1000)
        .unwrap();
...
fn test_p3_replay_first_class_citizen() {
    ...
    let verdict = checker
        .verify_execution_request(&safe_block, ExecutionMode::Replay, 1000)
        .unwrap();
...
fn test_cache_line_safety_rule() {
    ...
    let restrictions = RestrictionSet::new().with_safety_check(SafetyCheck::CacheLineSafety);

    assert!(!restrictions.is_empty());
    assert!(restrictions
        .required_checks
        .contains(&SafetyCheck::CacheLineSafety));
}
...
fn test_native_code_purity_constraint() {
    ...
    let restrictions = RestrictionSet::new().with_safety_check(SafetyCheck::NoSharedMutableState);

    assert!(restrictions
        .required_checks
        .contains(&SafetyCheck::NoSharedMutableState));
...
fn test_constitutional_compliance_configuration() {
    ...
    assert_eq!(
        config.locked_config.kill_switch_authority,
        KillSwitchAuthority::System
    );
    assert_eq!(
        config.locked_config.security_boundary,
        SecurityBoundary::Strict
    );
    assert_eq!(
        config.locked_config.phase_enforcement,
        PhaseEnforcement::Enabled
    );
}
...
fn test_full_constitutional_compliance() {
    ...
    let test_cases = vec![
        (
            create_test_block(1, ParallelSafety::Safe),
            ExecutionMode::Normal,
            1000,
            true,
        ),
        (
            create_test_block(2, ParallelSafety::Unsafe),
            ExecutionMode::Normal,
            1000,
            false,
        ),
        (
            create_test_block(3, ParallelSafety::Safe),
            ExecutionMode::Replay,
            1000,
            false,
        ),
        (
            create_test_block(4, ParallelSafety::ReductionOnly),
            ExecutionMode::Normal,
            1000,
            true,
        ),
