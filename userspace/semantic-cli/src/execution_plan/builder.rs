...
use super::{
    BlockId, BlockTerminator, ExecutionMetadata, ExecutionPlan, ExecutionPlanError, IRBlock,
    IRInstruction, ParallelSafety, RegisterId,
};
use crate::bcib::{
    BCIBInstruction, ComparisonOp, ContextInstruction, FilterExpression, OperandRef,
    QueryInstruction, Value,
};
use crate::execution_plan::dataflow::DataflowGraph;
use crate::normalizer::{InstructionGroup, NormalizedBCIB, NormalizedInstruction};
use crate::parallelism::{DefaultSafetyAnalyzer, ParallelSafetyAnalyzer};
...
impl IRBuilder {
    ...
    pub fn build_execution_plan(
    ...
    ) -> Result<ExecutionPlan, IRBuildError> {
        // Reset builder state
        self.reset();
        ...
        let entry_block =
            self.build_single_block(entry_block_id, ir_instructions, output_register)?;
        ...
        plan.validate()
            .map_err(|e| IRBuildError::ValidationFailed {
                reason: e.to_string(),
            })?;
    ...
    fn convert_instruction(
        ...
        match &normalized_inst.instruction {
            ...
            BCIBInstruction::ControlFlow(_) => {
                ...
                Err(IRBuildError::UnsupportedInstruction {
                    instruction: "Control flow instructions not yet implemented in Phase 2.3"
                        .to_string(),
                })
            }
        }
    ...
    fn convert_context_instruction(
        ...
        match context_inst {
            ...
            ContextInstruction::Return { .. } => {
                ...
                Ok(IRInstruction::Return { source_register })
            }
        }
    }
    ...
    fn build_single_block(
        ...
        let terminator = BlockTerminator::Return {
...
mod tests {
    ...
    use crate::normalizer::{NormalizedMetadata, RegisterAllocation};
    ...
    fn test_determinism_fingerprint() {
        ...
        let plan1 = builder1
            .build_execution_plan(normalized_bcib.clone())
            .unwrap();
