...
use super::{ExecutionPlan, IRInstruction, IRBlock, BlockTerminator, RegisterId, BlockId};
...
impl ExecutionPlanValidator {
    ...
    fn validate_structure(&self, plan: &ExecutionPlan, report: &mut ValidationReport) -> Result<(), ValidationError> {
    ...
    fn validate_block_structure(&self, block: &IRBlock, _report: &mut ValidationReport) -> Result<(), ValidationError> {
    ...
    fn validate_instruction(&self, instruction: &IRInstruction) -> Result<(), ValidationError> {
        match instruction {
            ...
            IRInstruction::LogicalOp { operand_registers, .. } => {
    ...
    fn validate_control_flow(&self, plan: &ExecutionPlan, report: &mut ValidationReport) -> Result<(), ValidationError> {
        ...
        let unreachable_blocks: Vec<_> = plan.blocks.iter()
    ...
    fn validate_register_usage(&self, plan: &ExecutionPlan, report: &mut ValidationReport) -> Result<(), ValidationError> {
        ...
        for block in &plan.blocks {
            for instruction in &block.instructions {
                ...
                for input_reg in instruction.input_registers() {
                    ...
                    if !defined_registers.contains(&input_reg) && self.config.strict_register_checking {
            ...
            match &block.terminator {
                ...
                BlockTerminator::Branch { condition, .. } => {
                    ...
                    if !defined_registers.contains(condition) && self.config.strict_register_checking {
                        return Err(ValidationError::UndefinedRegister { 
                            register: *condition,
                            block_id: block.id,
                        });
        ...
        let unused_registers: Vec<_> = defined_registers.iter()
    ...
    fn validate_dataflow(&self, plan: &ExecutionPlan, report: &mut ValidationReport) -> Result<(), ValidationError> {
        plan.dataflow_graph.validate()
    ...
    fn validate_determinism(&self, plan: &ExecutionPlan, report: &mut ValidationReport) -> Result<(), ValidationError> {
    ...
    fn validate_architectural_compliance(&self, plan: &ExecutionPlan, report: &mut ValidationReport) -> Result<(), ValidationError> {
    ...
    fn can_reach_self(&self, plan: &ExecutionPlan, start_block: BlockId, visited: &mut HashSet<BlockId>) -> bool {
...
impl ValidationReport {
    ...
    pub fn summary(&self) -> String {
        if self.is_valid() {
            format!("ExecutionPlan validation PASSED ({} warnings)", self.warnings.len())
        } else {
...
pub enum ValidationError {
    ...
    TerminatorInInstructions { block_id: BlockId, instruction_index: usize },
    ...
    UndefinedRegister { register: RegisterId, block_id: BlockId },
    
    #[error("Register {register} redefined in block {block_id}")]
    RegisterRedefinition { register: RegisterId, block_id: BlockId },
    ...
    NonDeterministicInstruction { block_id: BlockId, instruction: String },
...
mod tests {
    ...
    fn test_valid_execution_plan() {
        ...
        assert_eq!(report.summary(), "ExecutionPlan validation PASSED (0 warnings)");
    }
    ...
    fn test_terminator_in_instructions() {
        ...
        let result = validator.validate_block_structure(&invalid_block, &mut ValidationReport::new());
        assert!(result.is_err());
    }
    ...
    fn test_validation_report() {
        ...
        assert_eq!(report.summary(), "ExecutionPlan validation PASSED (1 warnings)");
    }
}