...
use semantic_cli::parallelism::{
    ...
    RayonParallelExecutor, ParallelExecutor, DefaultMetricsCollector, MetricsCollector,
...
fn benchmark_parallel_vs_sequential(c: &mut Criterion) {
    ...
    for (size_name, size) in sizes {
        ...
        group.bench_with_input(
            ...
            |b, data| {
                b.iter(|| {
                    black_box(execute_sequential(black_box(data)))
                })
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, data| {
                b.iter(|| {
                    ...
                    let indexed_results = executor.execute_parallel(
                        black_box(&block),
...
mod performance_tests {
    use super::*;
    use semantic_cli::parallelism::{ExecutionPhase, MIN_NET_SPEEDUP, MAX_OVERHEAD_RATIO};
    ...
    fn test_net_speedup_threshold() {
        ...
        if num_cpus::get() > 1 && data.len() >= LARGE_DATASET_SIZE {
            println!("Net speedup: {:.2}x (threshold: {:.2}x)", net_speedup, MIN_NET_SPEEDUP);
            // Relaxed assertion for CI environments
            assert!(net_speedup >= 1.0, "Net speedup should be at least 1.0x, got {:.2}x", net_speedup);
        }
    }
