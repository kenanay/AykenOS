...
use semantic_cli::parallelism::{
    AdaptiveDecisionEngine, ContiguousPartitioner, DataPartitioner, DefaultDecisionEngine,
    DefaultMetricsCollector, DeterministicMerger, ExecutionConfig, ImmutableContext,
    MetricsCollector, ParallelExecutor, RayonParallelExecutor, StableIndexMerger,
};
...
fn create_large_dataset(size: usize) -> Vec<Value> {
    (0..size)
        .map(|i| Value::Number(if i % 3 == 0 { i as f64 } else { -(i as f64) }))
        .collect()
}
...
fn compare_performance(
    ...
    println!(
        "      Overhead ratio: {:.1}%",
        metrics.ordering_overhead_ratio() * 100.0
    );
...
fn verify_correctness(
    ...
    let verification_result =
        execute_with_verification(block, &verification_data, context, &executor, &merger)?;
...
fn demonstrate_adaptive_decisions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        decision_engine.blacklist_size()
    );
    println!(
        "      Block {} blacklisted: {}",
        block.id,
