...
use semantic_cli::execution_plan::{
    IRBlock, IRInstruction, BlockTerminator, ParallelSafety
};
use semantic_cli::parallelism::{
    ContiguousPartitioner, StableIndexMerger, RayonParallelExecutor,
    ImmutableContext, ExecutionConfig,
    verification::{
        execute_with_verification, DefaultVerificationExecutor, VerificationExecutor,
        VerificationResult, VerificationDiagnostics, ValueMismatch
    }
};
use semantic_cli::execution_plan::{ExecutionPlan, ExecutionMetadata};
...
fn demonstrate_correct_verification() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match result {
        VerificationResult::Match { result, sequential_time, parallel_time, verification_overhead } => {
...
fn demonstrate_determinism_violation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match mismatch_result {
        VerificationResult::Mismatch { parallel_result, sequential_result, diagnostics } => {
            println!("   ❌ Verification FAILED!");
            println!("      Parallel result length: {}", parallel_result.len());
            println!("      Sequential result length: {}", sequential_result.len());
            
            print_diagnostics(&diagnostics);
            
            // Show detailed comparison
            println!("   🔍 Detailed Comparison:");
            for (i, (par, seq)) in parallel_result.iter().zip(sequential_result.iter()).enumerate() {
                if par != seq {
                    println!("      Index {}: parallel={:?}, sequential={:?} ❌", i, par, seq);
                } else {
                    println!("      Index {}: {:?} ✅", i, par);
                }
            }
    ...
    for size in sizes {
        ...
        match result {
            ...
            VerificationResult::Mismatch { .. } => {
                println!("   {:>8} | ERROR      | ERROR      | ERROR   | Failed",
                         format_size(size));
            }
        }
    }
...
fn demonstrate_large_dataset_verification() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   📊 Verifying large dataset with {} elements", large_data.len());
    ...
    match result {
        VerificationResult::Match { result, sequential_time, parallel_time, verification_overhead } => {
            ...
            println!("      Verification overhead: {:.1}%", overhead_ratio * 100.0);
...
fn demonstrate_mixed_data_verification() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   🎭 Verifying mixed data types ({} elements)", mixed_data.len());
...
fn print_diagnostics(diagnostics: &VerificationDiagnostics) {
    ...
    println!("         First mismatch index: {:?}", diagnostics.first_mismatch_index);
...
fn analyze_diagnostics(diagnostics: &VerificationDiagnostics) {
    ...
    let mismatch_ratio = diagnostics.value_mismatches.len() as f64 / diagnostics.input_data.len() as f64;
