...
use semantic_cli::parallelism::{
    measure_execution, operations, DefaultReductionHandler, ReductionHandler, ReductionType,
};
...
fn create_numeric_dataset(size: usize) -> Vec<Value> {
    (1..=size).map(|i| Value::Number(i as f64)).collect()
}
...
fn demonstrate_sum_aggregation(data: &[Value]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (parallel_result, parallel_time) =
        measure_execution(|| operations::sum(&handler, numeric_values.clone()));
...
fn demonstrate_count_aggregation(data: &[Value]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (parallel_counts, parallel_time) = measure_execution(|| count_value_types_parallel(data));
...
fn demonstrate_min_max_aggregation(data: &[Value]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (parallel_min_result, min_time) =
        measure_execution(|| operations::min(&handler, subset.clone()));

    let (parallel_max_result, max_time) =
        measure_execution(|| operations::max(&handler, subset.clone()));
...
fn demonstrate_variance_aggregation(data: &[Value]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Sequential variance
    let (sequential_variance, sequential_time) =
        measure_execution(|| calculate_variance_sequential(&subset));

    println!("   ⏱️  Sequential time: {:?}", sequential_time);
    println!("   📊 Sequential variance: {:.2}", sequential_variance);

    // Parallel variance
    let (parallel_variance, parallel_time) =
        measure_execution(|| calculate_variance_parallel(&subset));
...
fn demonstrate_string_concatenation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   ⏱️  Sequential time: {:?}", sequential_time);
    println!(
        "   📝 Sequential result length: {} chars",
        sequential_result.len()
    );

    // Parallel concatenation using non-commutative reduction
    let indexed_strings: Vec<(usize, Value)> = strings.into_iter().enumerate().collect();

    let (parallel_result, parallel_time) =
        measure_execution(|| operations::concat(&handler, indexed_strings));
...
fn demonstrate_grouped_aggregation(data: &[Value]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (groups, grouping_time) = measure_execution(|| group_by_type_parallel(data));
...
fn compare_aggregation_performance(data: &[Value]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   📈 Performance comparison for {} elements:",
        subset.len()
    );
    ...
    // Count comparison
    let (_, seq_count_time) = measure_execution(|| count_value_types(&subset));

    let (_, par_count_time) = measure_execution(|| count_value_types_parallel(&subset));

    let count_speedup = seq_count_time.as_secs_f64() / par_count_time.as_secs_f64();

    println!(
        "   Count        | {:>8.2}ms | {:>8.2}ms | {:>5.2}x",
