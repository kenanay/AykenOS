...
use semantic_cli::execution_plan::{IRBlock, IRInstruction, BlockTerminator, ParallelSafety};
use semantic_cli::parallelism::{
    ...
    ImmutableContext, ExecutionConfig, DefaultReductionHandler, operations
};
...
fn arbitrary_safe_ir_block() -> impl Strategy<Value = IRBlock> {
    (0u16..100, prop::collection::vec(arbitrary_ir_instruction(), 1..10))
...
fn arbitrary_ir_instruction() -> impl Strategy<Value = IRInstruction> {
    prop_oneof![
        (prop::string::string_regex("[a-z]+").unwrap(), 0u16..10)
            .prop_map(|(context_id, target_register)| {
                IRInstruction::LoadContext {
                    context_id,
                    target_register,
                }
            }),
        (0u16..10, prop::string::string_regex("[a-z]+").unwrap(), 0u16..10)
        ...
        (arbitrary_value(), 0u16..10)
            .prop_map(|(value, target_register)| {
...
fn arbitrary_value() -> impl Strategy<Value = Value> {
    prop_oneof![
        ...
        prop::string::string_regex("[a-zA-Z0-9 ]*").unwrap().prop_map(Value::String),
...
mod property_validation_tests {
    ...
    fn test_property_generators() {
        ...
        let block = strategy.new_tree(&mut proptest::test_runner::TestRunner::default())
            .unwrap()
            .current();
    ...
    fn test_dataset_generator() {
        ...
        let data = strategy.new_tree(&mut proptest::test_runner::TestRunner::default())
            .unwrap()
            .current();
