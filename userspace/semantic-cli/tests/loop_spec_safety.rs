...
mod safety_analysis_tests {
    ...
    fn test_io_side_effect_detection() {
        ...
        let result = analyzer.analyze_loop_safety(file_io_body, &context).unwrap();
        ...
        let has_file_io = result.side_effects.iter().any(|effect| {
            matches!(effect, semantic_cli::loop_engine::safety_analyzer::SideEffect::IOOperation { 
                operation_type: semantic_cli::loop_engine::safety_analyzer::IOOperationType::FileSystem, 
                .. 
            })
        });
        assert!(has_file_io);
    ...
    fn test_console_io_detection() {
        ...
        let result = analyzer.analyze_loop_safety(console_body, &context).unwrap();
        ...
        let has_console_io = result.side_effects.iter().any(|effect| {
            matches!(effect, semantic_cli::loop_engine::safety_analyzer::SideEffect::IOOperation { 
                operation_type: semantic_cli::loop_engine::safety_analyzer::IOOperationType::Console, 
                .. 
            })
        });
    ...
    fn test_network_io_detection() {
        ...
        let result = analyzer.analyze_loop_safety(network_body, &context).unwrap();
        ...
        let has_network_io = result.side_effects.iter().any(|effect| {
            matches!(effect, semantic_cli::loop_engine::safety_analyzer::SideEffect::IOOperation { 
                operation_type: semantic_cli::loop_engine::safety_analyzer::IOOperationType::Network, 
                .. 
            })
        });
    ...
    fn test_database_io_detection() {
        ...
        let has_db_io = result.side_effects.iter().any(|effect| {
            matches!(effect, semantic_cli::loop_engine::safety_analyzer::SideEffect::IOOperation { 
                operation_type: semantic_cli::loop_engine::safety_analyzer::IOOperationType::Database, 
                .. 
            })
        });
        assert!(has_db_io);
    ...
    fn test_external_mutation_detection() {
        ...
        let result = analyzer.analyze_loop_safety(mutation_body, &context).unwrap();
    ...
    fn test_loop_carried_dependency_detection() {
        ...
        let result = analyzer.analyze_loop_safety(dependency_body, &context).unwrap();
        ...
        let has_dependency = result.dependencies.iter().any(|dep| {
            ...
            matches!(dep.dependency_type, semantic_cli::loop_engine::safety_analyzer::DependencyType::ReadAfterWrite)
        });
    ...
    fn test_multiple_side_effects() {
        ...
        let has_file_io = result.side_effects.iter().any(|e| {
            matches!(e, semantic_cli::loop_engine::safety_analyzer::SideEffect::IOOperation { 
                operation_type: semantic_cli::loop_engine::safety_analyzer::IOOperationType::FileSystem, 
                .. 
            })
        });
        ...
        let has_mutation = result.side_effects.iter().any(|e| {
            matches!(e, semantic_cli::loop_engine::safety_analyzer::SideEffect::ExternalMutation { .. })
        });
        let has_call = result.side_effects.iter().any(|e| {
            matches!(e, semantic_cli::loop_engine::safety_analyzer::SideEffect::ExternalCall { .. })
        });
    ...
    fn test_known_safe_functions() {
        ...
        let result = analyzer.analyze_loop_safety(safe_function_body, &context).unwrap();
