...
use semantic_cli::bcib::{
    LoopInstruction, LoopID, LoopConfig, LoopRange, Value, ValueType, BudgetMeasurement,
};
use semantic_cli::loop_engine::{LoopExecutor, LoopBodyFn};
...
fn test_budget_timeout_enforcement() {
    ...
    let body_fn: LoopBodyFn = Box::new(|accumulator, _| {
        if let Value::Number(acc) = accumulator {
            Ok(semantic_cli::loop_engine::LoopBodyResult::Normal(Value::Number(acc + 1.0)))
        } else {
...
fn test_for_loop_basic_execution_no_timeout() {
    ...
    let body_fn: LoopBodyFn = Box::new(|accumulator, iteration| {
        if let Value::Number(acc) = accumulator {
            Ok(semantic_cli::loop_engine::LoopBodyResult::Normal(Value::Number(acc + iteration as f64)))
        } else {
            Err(SemanticCLIError::execution_error("Invalid accumulator type", ErrorCode::E500))
        }
    });
...
fn test_type_safety_enforcement() {
    ...
    let body_fn: LoopBodyFn = Box::new(|_accumulator, iteration| {
        if iteration == 0 {
            Ok(semantic_cli::loop_engine::LoopBodyResult::Normal(Value::Number(42.0))) // Valid
        } else {
            Ok(semantic_cli::loop_engine::LoopBodyResult::Normal(Value::String("invalid".to_string()))) // Type change - should fail
        }
    });
