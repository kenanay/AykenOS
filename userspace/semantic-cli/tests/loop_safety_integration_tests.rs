...
use semantic_cli::loop_engine::{
    LoopEngine, LoopAnalysisContext, SafetyClass, 
    safety_analyzer::{SideEffect, IOOperationType, VariableScope, LoopCarriedDependency, DependencyType}
};
...
fn test_loop_engine_safety_integration() {
    ...
    let result = loop_engine.analyze_loop_safety(safe_body, &context).unwrap();
...
fn test_io_side_effect_detection() {
    ...
    let result = loop_engine.analyze_loop_safety(file_io_body, &context).unwrap();
...
fn test_external_mutation_detection() {
    ...
    let result = loop_engine.analyze_loop_safety(mutation_body, &context).unwrap();
...
fn test_external_call_detection() {
    ...
    let result = loop_engine.analyze_loop_safety(call_body, &context).unwrap();
...
fn test_loop_carried_dependency_detection() {
    ...
    let result = loop_engine.analyze_loop_safety(dependency_body, &context).unwrap();
...
fn test_multiple_side_effects() {
    ...
    let result = loop_engine.analyze_loop_safety(complex_body, &context).unwrap();
...
fn test_safety_analysis_caching() {
    ...
    // First analysis
    let result1 = loop_engine.analyze_loop_safety(loop_body, &context).unwrap();
    let cache_stats1 = loop_engine.get_safety_cache_stats();
    
    // Second analysis - should use cache
    let result2 = loop_engine.analyze_loop_safety(loop_body, &context).unwrap();
...
fn test_known_safe_functions() {
    ...
    let result = loop_engine.analyze_loop_safety(safe_function_body, &context).unwrap();
...
fn test_cache_clearing() {
    ...
    loop_engine.analyze_loop_safety(loop_body, &context).unwrap();
...
fn test_conservative_mode_behavior() {
    ...
    let unknown_body = "result = completely_unknown_function(i); accumulator = accumulator + result";
    let result = loop_engine.analyze_loop_safety(unknown_body, &context).unwrap();
...
fn test_network_io_detection() {
    ...
    let result = loop_engine.analyze_loop_safety(network_body, &context).unwrap();
    
    assert_eq!(result.classification, SafetyClass::Unsafe);
    
    // Check that network I/O was detected
    let has_network_io = result.side_effects.iter().any(|effect| {
        matches!(effect, SideEffect::IOOperation { operation_type: IOOperationType::Network, .. })
    });
...
fn test_database_io_detection() {
    ...
    let has_db_io = result.side_effects.iter().any(|effect| {
        matches!(effect, SideEffect::IOOperation { operation_type: IOOperationType::Database, .. })
    });
    assert!(has_db_io);
}
...
fn test_system_call_detection() {
    ...
    let syscall_body = "result = system_call('ls', ['-la']); accumulator = accumulator + result.exit_code";
    let result = loop_engine.analyze_loop_safety(syscall_body, &context).unwrap();
    
    assert_eq!(result.classification, SafetyClass::Unsafe);
    
    // Check that system call was detected
    let has_syscall = result.side_effects.iter().any(|effect| {
        matches!(effect, SideEffect::IOOperation { operation_type: IOOperationType::SystemCall, .. })
    });
    assert!(has_syscall);
}