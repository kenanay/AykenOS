...
use semantic_cli::loop_engine::{
    D2LoopIntegration, DeterministicPartitioner, LoopEngine, SafetyAnalysisResult, SafetyClass,
    StableIndexMapper,
};
...
fn test_stable_index_mapping_foreach_loop() {
    ...
    for i in 0..4 {
        let data1 = mapper
            .get_input_data_for_iteration(&foreach_loop, i)
            .unwrap();
        let data2 = mapper
            .get_input_data_for_iteration(&foreach_loop, i)
            .unwrap();
...
fn test_mapping_strategy_analysis() {
    ...
    assert_eq!(
        while_strategy,
        semantic_cli::loop_engine::IndexMappingStrategy::Iteration
    );
}
...
fn test_constitutional_compliance_stable_index_mapping() {
    ...
    for global_iteration in 0..100 {
        let data1 = mapper
            .get_input_data_for_iteration(&for_loop, global_iteration)
            .unwrap();
        let data2 = mapper
            .get_input_data_for_iteration(&for_loop, global_iteration)
            .unwrap();
...
fn test_property_determinism_across_calls() {
    ...
    let test_cases = [(100, 2), (500, 4), (1000, 8), (5000, 16)];
