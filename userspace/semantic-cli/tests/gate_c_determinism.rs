...
use semantic_cli::ir_planner::{ExecutionResult, IRExecutor};
...
fn test_proof1_ir_execution_determinism() {
    ...
    for (i, result) in results.iter().enumerate() {
        assert_eq!(
            result.execution_steps, first_steps,
            "Execution {} step count differs from first execution",
            i
        );
    }
...
fn test_proof1_determinism_with_filter() {
    ...
    for i in 1..results.len() {
        assert_eq!(
            results[0].execution_steps, results[i].execution_steps,
...
fn test_proof1_determinism_with_branch() {
    ...
    for _ in 0..3 {
        ...
        let result = executor
            .execute(ir.clone())
            .expect("Branch execution failed");
        results.push(result);
    }
...
fn test_proof2_replay_produces_same_result() {
    ...
    assert_eq!(
        trace.execution_plan_fingerprint, expected_fingerprint,
...
fn test_proof2_multiple_replays_identical() {
    ...
    let (result1, trace1) = executor1
        .execute_with_replay(ir.clone())
        .expect("First replay failed");

    let mut executor2 = IRExecutor::new();
    let (result2, trace2) = executor2
        .execute_with_replay(ir)
        .expect("Second replay failed");

    // Traces must be structurally identical
    assert_eq!(
        trace1.total_steps, trace2.total_steps,
        "Trace step counts differ"
    );
    assert_eq!(
        trace1.execution_plan_fingerprint, trace2.execution_plan_fingerprint,
        "Trace fingerprints differ"
    );

    // Results must be identical
    assert_eq!(
        result1.execution_steps, result2.execution_steps,
        "Result step counts differ"
    );
}
...
fn test_proof3_replay_trace_integrity() {
    ...
    assert!(
        !trace.execution_plan_fingerprint.is_empty(),
        "Fingerprint missing"
    );
    ...
    for (i, step) in trace.steps.iter().enumerate() {
        ...
        assert!(
            step.step_number > 0,
            "Step number should be > 0 at index {}",
            i
        );
        ...
        assert!(
            ...
            "Invalid execution step at index {}",
            i
        );
    }
}
    ...
    for i in 0..trace.steps.len() {
        ...
        if i > 0 {
            ...
            assert!(
                ...
                "Step ordering violated at index {}",
                i
            );
        }
    }
...
fn test_proof3_replay_trace_summary() {
    ...
    assert_eq!(
        summary.total_steps, trace.total_steps,
        "Summary step count mismatch"
    );
...
fn test_proof4_fingerprint_covers_instruction_order() {
    ...
    assert_ne!(
        fp1, fp2,
        "Different instruction orders must produce different fingerprints"
    );
}
