...
fn test_validator_with_valid_commands() {
    ...
    for command in test_cases {
        ...
        assert!(result.is_ok(), "Validation failed for valid command: {} - Error: {:?}", command, result.err());
    }
}
...
fn test_validator_with_invalid_contexts() {
    ...
    for (command, expected_code) in test_cases {
        ...
        let result = validator.validate(&ast);
        assert!(result.is_err(), "Validation should fail for invalid context: {}", command);
...
fn test_validator_nested_commands() {
    ...
    for command in test_cases {
        ...
        let result = validator.validate(&ast);
        assert!(result.is_ok(), "Nested command validation should pass for: {} - Error: {:?}", command, result.err());
    }
}
...
fn test_validator_performance() {
    ...
    assert!(avg_per_validation.as_millis() < 1, "Validation too slow: {:?}", avg_per_validation);
}
...
fn test_validator_all_contexts() {
    ...
    for context in contexts {
        ...
        let result = validator.validate(&ast);
        assert!(result.is_ok(), "Context validation should pass for: {} - Error: {:?}", context, result.err());
    }
}