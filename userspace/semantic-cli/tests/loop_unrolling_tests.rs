...
use semantic_cli::bcib::{
    LoopInstruction, LoopID, LoopConfig, Value, ValueType, LoopRange, 
    CollectionType, OperandRef, BCIBSequence
};
use semantic_cli::loop_engine::{
    LoopUnroller, UnrollResult, UnrollSkipReason, UnrollConfig, LoopEngine
};
        ...
        match result {
            UnrollResult::Unrolled { iteration_count, unrolled_sequence } => {
                assert_eq!(iteration_count, expected_iterations, 
                    "Loop [{}, {}, {}] should have {} iterations", start, end, step, expected_iterations);
                ...
                assert_eq!(unrolled_sequence.instructions.len(), expected_instruction_count as usize,
                    "Unrolled sequence should have {} instructions", expected_instruction_count);
            }
            UnrollResult::NotUnrolled { reason, .. } => {
                panic!("Expected unrolling for small loop [{}, {}, {}] but got: {}", 
    ...
    for (start, end, step, expected_iterations) in test_cases {
        ...
        match result {
            ...
            UnrollResult::Unrolled { .. } => {
                panic!("Expected no unrolling for large loop [{}, {}, {}]", start, end, step);
            }
        }
    }
}
...
fn test_requirement_4_2_sequential_ir_instructions() {
    ...
    match result {
        UnrollResult::Unrolled { iteration_count, unrolled_sequence } => {
            ...
            assert!(unrolled_sequence.validate().is_ok(), 
...
fn test_foreach_literal_collection_unrolling() {
    ...
    match result {
        ...
        UnrollResult::NotUnrolled { reason, .. } => {
            panic!("Expected unrolling for SortedMap literal but got: {}", reason);
        }
    }
}
...
fn test_complex_range_patterns() {
    ...
    for (start, end, step, expected_iterations, description) in test_cases {
        ...
        match result {
            UnrollResult::Unrolled { iteration_count, unrolled_sequence } => {
                assert_eq!(iteration_count, expected_iterations, 
                    "Test case '{}' should have {} iterations", description, expected_iterations);
                
                let expected_instruction_count = expected_iterations * 2;
                assert_eq!(unrolled_sequence.instructions.len(), expected_instruction_count as usize,
                    "Test case '{}' should generate {} instructions", description, expected_instruction_count);
            }
            UnrollResult::NotUnrolled { reason, .. } => {
                panic!("Expected unrolling for test case '{}' but got: {}", description, reason);
            }
        }
    }
}
...
fn test_property_4_loop_unrolling_semantic_preservation() {
    ...
    for loop_instruction in test_loops {
        ...
        match result {
            UnrollResult::Unrolled { unrolled_sequence, .. } => {
