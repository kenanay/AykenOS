...
use semantic_cli::loop_engine::{
    LoopAnalysisContext, LoopEngine, ParallelizationDecision, SafetyClass,
};
...
fn test_parallelization_trigger_workflow() {
    ...
    let safety_result = loop_engine
        .analyze_loop_safety("accumulator = accumulator + i * 2", &context)
        .unwrap();
    ...
    let safety_result = loop_engine
        .analyze_loop_safety("accumulator = accumulator + 1", &context)
        .unwrap();
    ...
    let safety_result = loop_engine
        .analyze_loop_safety("accumulator = accumulator + item", &foreach_context)
        .unwrap();
    ...
    assert!(decision
        .sequential_reason()
        .unwrap()
        .contains("Dynamic iteration count"));
}
...
fn test_parallelization_benefit_estimation() {
    ...
    if let ParallelizationDecision::Parallel {
        estimated_benefit, ..
    } = decision
    {
...
fn test_safety_analysis_caching_integration() {
    ...
    let safety_result1 = loop_engine
        .analyze_loop_safety("accumulator = accumulator + i", &context)
        .unwrap();

    // Second analysis - should be a cache hit
    let safety_result2 = loop_engine
        .analyze_loop_safety("accumulator = accumulator + i", &context)
        .unwrap();
