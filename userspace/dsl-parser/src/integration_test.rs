...
mod integration_tests {
    ...
    fn test_complete_hierarchical_workflow() {
        ...
        let result = parser
            .parse_command(">> add {\"id\":1,\"name\":\"Alice\",\"age\":30}")
            .unwrap();
        match result.command {
            Command::Add { payload } => {
                assert_eq!(payload, "{\"id\":1,\"name\":\"Alice\",\"age\":30}")
            }
            _ => panic!("Expected Add command"),
        }

        let result = parser
            .parse_command(">> query filter=\"age > 25\"")
            .unwrap();
        match result.command {
        ...
        let result = parser
            .parse_command(
                ">[ ] add {\"id\":2,\"name\":\"Bob\"} | query filter=\"name like 'B%'\" | list",
            )
            .unwrap();
        match result.command {
            Command::Batch(items) => {
    ...
    fn test_all_context_operations() {
        ...
        for (cmd, expected_type) in operations {
            ...
            match (&dispatch.command, expected_type) {
                ...
                (Command::Exit, "Exit") => {}
                _ => panic!(
                    "Command type mismatch for '{}': expected {}",
                    cmd, expected_type
                ),
            }
        }
    ...
    fn test_batch_operation_parsing() {
        ...
        let batch_tests = vec![
            ...
            (
                ">[ ] query filter=\"test\" | add {\"id\":1} | list",
                vec!["query filter=\"test\"", "add {\"id\":1}", "list"],
            ),
        ];
            ...
            match result.command {
                Command::Batch(items) => {
                    assert_eq!(
                        items.len(),
                        expected_items.len(),
                        "Batch command '{}' item count mismatch",
                        cmd
                    );
                    for (i, expected) in expected_items.iter().enumerate() {
                        assert_eq!(
                            items[i], *expected,
                            "Batch command '{}' item {} mismatch",
                            cmd, i
                        );
                    }
                }
    ...
    fn test_parameter_parsing_edge_cases() {
        ...
        assert!(parser
            ...
            .is_ok());
        assert!(parser
            .parse_command(">> create [complex:json,data:array]")
            .is_ok());

        // JSON with various formats
        assert!(parser
            .parse_command(">> add {\"simple\":\"value\"}")
            .is_ok());
        assert!(parser
            .parse_command(">> add { \"spaced\" : \"value\" }")
            .is_ok());
        assert!(parser
            .parse_command(">> add {\"nested\":{\"key\":\"value\"}}")
            .is_ok());

        // Query filters with various formats
        assert!(parser
            .parse_command(">> query filter=\"simple = 1\"")
            .is_ok());
        assert!(parser.parse_command(">> query \"complex filter\"").is_ok());
        assert!(parser
            .parse_command(">> query filter=\"name like 'test%'\"")
            .is_ok());
        ...
        assert!(parser
            .parse_command(">> ask \"Complex question with 'quotes' and symbols!\"")
            .is_ok());
