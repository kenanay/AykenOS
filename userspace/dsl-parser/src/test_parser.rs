...
mod tests {
    ...
    fn test_context_commands() {
        ...
        match result.command {
            Command::Add { payload } => assert_eq!(payload, "{\"id\":1,\"name\":\"Ahmet\",\"age\":34}"),
    ...
    fn test_error_cases() {
        ...
        // Test empty input
        assert!(matches!(parser.parse_command(""), Err(ParseError::EmptyInput)));
        assert!(matches!(parser.parse_command("   "), Err(ParseError::EmptyInput)));
        
        // Test invalid syntax
        assert!(matches!(parser.parse_command("invalid"), Err(ParseError::InvalidSyntax)));
    ...
    fn test_ai_commands() {
        ...
        let result = parser.parse_command(">> ask \"What is the weather today?\"").unwrap();
        match result.command {
    ...
    fn test_context_validation() {
        ...
        assert!(matches!(parser.parse_command("> random"), Err(ParseError::UnsupportedContext(_))));
    }
    ...
    fn test_json_validation() {
        ...
        assert!(parser.parse_command(">> add { \"name\": \"test\" }").is_ok());
        
        // Test invalid JSON-like format
        assert!(matches!(parser.parse_command(">> add invalid"), Err(ParseError::InvalidJson(_))));
        assert!(matches!(parser.parse_command(">> add [1,2,3]"), Err(ParseError::InvalidJson(_))));
    }
    ...
    fn test_schema_validation() {
        ...
        assert!(parser.parse_command(">> create [id:int,name:string,age:int]").is_ok());
        
        // Test invalid schema format
        assert!(matches!(parser.parse_command(">> create schema=invalid"), Err(ParseError::InvalidSchema(_))));
        assert!(matches!(parser.parse_command(">> create invalid"), Err(ParseError::InvalidSchema(_))));
    }
}