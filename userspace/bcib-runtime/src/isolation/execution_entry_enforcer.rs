...
use crate::isolation::execution_entry_context::{ExecutionEntryContext, SyscallOrigin, PrivilegeLevel};
use crate::isolation::fail_closed::FailClosedTermination;
use crate::isolation::kernel_syscall_validator::{KernelSyscallValidator, ExecutionRole, SyscallNumber};
...
impl ExecutionEntryEnforcer {
    ...
    pub fn validate_execution_entry(
        ...
        if entry_point == "SYS_V2_SUBMIT_EXECUTION" {
            ...
            let syscall_result = self.kernel_validator.validate_syscall(
                SyscallNumber::SysV2SubmitExecution as u64,
                context_id
            );
        ...
        match validation_result {
            ...
            EntryValidationResult::DirectInvocation { attempted_path, violation_type } => {
                let error_message = format!(
            ...
            EntryValidationResult::BypassAttempt { attempted_bypass, detection_method } => {
                let error_message = format!(
...
mod tests {
    ...
    fn invalid_syscall_entry_rejected_with_kernel_termination() {
        ...
        let event = harness
            .execute_expecting_termination(|| enforcer.validate_kernel_execution_entry(&invalid_context))
            .expect("invalid syscall entry must terminate");

        harness
            .verify_termination_event(&event, TerminationReason::KernelSafetyCritical, "KERNEL.SAFETY.CRITICAL")
    ...
    fn runtime_bridge_syscall_rejected_for_bcib_entry() {
        ...
        let invalid_context = ExecutionEntryContext::from_kernel_dispatcher(
            SyscallNumber::SysV2CapabilityCheck as u64, // Valid for Runtime Bridge, invalid for BCIB entry
            1234,
            5678,
            vec!["kernel_syscall_dispatcher".to_string()],
        );
    ...
    fn execution_bypass_detected_and_terminated() {
        ...
        let event = harness
            .execute_expecting_termination(|| enforcer.validate_kernel_execution_entry(&bypass_context))
            .expect("bypass attempt must terminate");

        harness
            .verify_termination_event(&event, TerminationReason::SecurityBoundaryViolation, "SECURITY.BOUNDARY.VIOLATION")
