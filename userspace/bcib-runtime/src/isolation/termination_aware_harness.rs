...
impl TerminationAwareHarness {
        ...
        F: FnOnce() -> Result<(), BcibError> + std::panic::UnwindSafe,
    {
        ...
        if events.len() > 1 {
            return Err(format!("Expected single termination but captured {}", events.len()));
        }
...
mod tests {
    ...
    fn harness_captures_security_boundary_violation() {
        ...
        let result = harness.execute_expecting_termination(|| {
            ...
            runtime.create_context_from_syscall(plan, caps, invalid_context)
                .map(|_| ())
        });
    ...
    fn harness_detects_unexpected_success() {
        ...
        assert!(result.unwrap_err().contains("Expected termination but none was captured"));
    }
    ...
    fn harness_verifies_successful_execution() {
        ...
        assert!(result.is_ok(), "Valid kernel entry should succeed: {:?}", result);
    }
}