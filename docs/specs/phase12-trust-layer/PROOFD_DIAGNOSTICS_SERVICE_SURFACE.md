...
- a local `POST /verify/bundle` execution family may delegate to verifier-core with explicit `bundle_path`, `policy_path`, `registry_path`, `receipt_mode`, `receipt_signer`, optional `diversity_binding`, optional Stage-2 `replay_boundary_binding`, optional `trust_reuse_runtime_binding`, optional fallback `trust_reuse_binding`, and `run_id` binding while keeping diagnostics endpoints read-only
...
- `trust_reuse_flow_source.json` now prefers run-local `trust_reuse_runtime_surface.json` when `proofd` materializes native trust-reuse runtime evidence from explicit evaluator inputs, otherwise prefers bundle-native `reports/trust_reuse_runtime_surface.json`, emits `NO_REUSABLE_EVENTS` when native trust-reuse was evaluated but yielded no reusable path, and only falls back to explicit `trust_reuse_binding` when no native runtime surface is present
