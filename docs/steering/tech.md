...
make ci-freeze-guard                  # prerequisite: strict freeze mode guard
make preflight-mode-guard             # prerequisite: validation mode guard
# The numbered list below contains the 40 gate/cluster targets.
...
make ci-gate-spec-purity              # 11. Normative specification purity (MUST pass)
make ci-gate-drift-activation         # 12. Drift blocking activation requirement (MUST pass)
make ci-gate-structural-abi           # 13. Structural ABI check (MUST pass)
make ci-gate-runtime-marker-contract  # 14. Runtime marker contract (MUST pass)
make ci-gate-user-bin-lock            # 15. User binary lock (MUST pass)
make ci-gate-embedded-elf-hash        # 16. Embedded ELF hash integrity (MUST pass)
make ci-gate-performance              # 17. Performance regression check (MUST pass)
...
make ci-gate-ring3-user-leaf-rule     # 18. Ring3 executable user-leaf rule (local deterministic rule authority)
make ci-gate-ring3-execution-phase10a2 # 19. Broader Ring3 Phase10-A2 strict/global authority surface
make ci-gate-syscall-semantics-phase10b # 20. Syscall semantics Phase 10-B (MUST pass)
make ci-gate-low-half-kheap-scaffold  # 21. Low-half kheap scaffold proof (MUST pass)
make $(PHASE10C_FREEZE_GATE)          # 22. Phase 10-C gate (conditional freeze gate)
make ci-gate-mailbox-capability-negative # 23. Mailbox capability negative test (MUST pass)
make ci-gate-workspace                # 24. Workspace integrity (MUST pass)
make ci-gate-syscall-v2-runtime       # 25. Syscall v2 runtime validation (MUST pass)
make ci-gate-sched-bridge-runtime     # 26. Scheduler bridge runtime validation (MUST pass)
make ci-gate-behavioral-suite         # 27. Behavioral suite (MUST pass)
make ci-gate-policy-accept            # 28. Policy accept proof (MUST pass)
make ci-gate-alias-proof              # 29. Alias proof runtime check (MUST pass)
make ci-kill-switch-phase13           # 30. Phase-13 kill-switch gates (distributed verification)
make ci-gate-determinism-replay-consistency # 31. Determinism replay consistency (MUST pass)
make ci-gate-bcib-v3-core             # 32. BCIB v3 core workstream gate (MUST pass)
make ci-gate-toolchain-opcode-registry # 33. Toolchain opcode registry gate (MUST pass)
make ci-gate-capability-manager       # 34. Capability manager gate (MUST pass)
make ci-gate-proofd-observability-boundary # 35. proofd observability boundary gate (MUST pass)
make ci-gate-dsl-bcib-contract        # 36. DSL to BCIB contract gate (MUST pass)
make ci-gate-semantic-cli-contract    # 37. Semantic CLI contract gate (MUST pass)
make ci-gate-data-runtime-bcib        # 38. Data runtime BCIB gate (MUST pass)
make ci-gate-ai-runtime-boundary      # 39. AI runtime boundary gate (MUST pass)
make ci-gate-bcib-stub-determinism    # 40. BCIB stub determinism gate (MUST pass)
