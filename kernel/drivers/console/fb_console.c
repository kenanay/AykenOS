...
static void scroll_if_needed(void)
{
    ...
    // Yukarı kopyala
    // Optimize: 64-bit kopyalama (8 byte birden)
    uint64_t copy_size = total_bytes - bytes_per_line;
    uint64_t *dst_64 = (uint64_t*)base;
    uint64_t *src_64 = (uint64_t*)(base + bytes_per_line);
    uint64_t n_64 = copy_size / 8;

    for (uint64_t i = 0; i < n_64; ++i) {
        dst_64[i] = src_64[i];
    }
    // Kalan byte'lar (hizalama dışı kalanlar)
    for (uint64_t i = n_64 * 8; i < copy_size; ++i) {
        base[i] = base[i + bytes_per_line];
    }

    // Son satırı temizle
    uint8_t *last = base + copy_size;
    uint64_t *last_64 = (uint64_t*)last;
    uint64_t clear_n_64 = bytes_per_line / 8;

    for (uint64_t i = 0; i < clear_n_64; ++i) {
        last_64[i] = 0;
    }
    // Kalan byte'lar
    for (uint64_t i = clear_n_64 * 8; i < bytes_per_line; ++i) {
        last[i] = 0;
    }
...
// Convenience wrapper for 64-bit hex (same as fb_print_hex)
void fb_print_hex64(uint64_t v)
{
    fb_print_hex(v);
}
