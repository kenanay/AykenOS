...
typedef struct {
     ...
     *  - `abi_version` ile bootloader/kernel sözleşmesi doğrulanır
     *  - `flags` bootloader tarafından sağlanan yetenekleri belirtir
     * -----------------------------------------------------------------*/
    uint32_t abi_version;        // Boot ABI sürümü
    uint32_t flags;              // Boot özellik bayrakları (bitmask)
