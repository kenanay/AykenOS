#!/bin/bash
# Test script for VCP Fail-Closed Enforcement (Task 4)
#
...
echo "=========================================="
echo "VCP Fail-Closed Integration Runner"
echo "=========================================="
echo ""
echo "Building kernel with VCP fail-closed property tests enabled..."
echo ""
...
if [ "$run_status" -ne 0 ] && [ "$run_status" -ne 124 ]; then
    ...
    show_log_matches "VCP_FAIL_CLOSED_TESTS|CRITICAL FAILURE|\\[FAIL\\]"
    exit 1
fi

if log_has "VCP_FAIL_CLOSED_TESTS FAILED|CRITICAL FAILURE|\\[FAIL\\] VCP fail-closed"; then
    echo "ERROR: VCP fail-closed integration failed." >&2
    show_log_matches "VCP_FAIL_CLOSED_TESTS|CRITICAL FAILURE|\\[FAIL\\]"
    exit 1
fi

if ! log_has "VCP_FAIL_CLOSED_TESTS PASSED"; then
    echo "ERROR: VCP fail-closed PASS marker was not found in $LOG_PATH." >&2
    show_log_matches "VCP_FAIL_CLOSED_TESTS|LATE_INIT|BOOT_VALIDATION"
    exit 1
fi
...
do
    if ! log_has "$marker"; then
        echo "ERROR: expected fail-closed marker not found: $marker" >&2
        show_log_matches "VCP_FAIL_CLOSED|VCP_FAIL_CLOSED_TESTS|CRITICAL FAILURE|\\[FAIL\\]"
        exit 1
    fi
done

echo "Boot evidence:"
show_log_matches "VCP_FAIL_CLOSED_TESTS PASSED|VCP_FAIL_CLOSED"
echo ""
echo "PASS: VCP fail-closed integration test passed."
echo ""
