...
usage() {
  cat <<'USAGE'
Usage:
  scripts/ci/gate_dlt_monotonicity.sh \
    --evidence-dir evidence/run-<id>/gates/dlt-monotonicity \
    --eti-evidence evidence/run-<id>/gates/eti

Exit codes:
  0: pass
  2: DLT monotonicity contract failure
...
VALIDATOR="${ROOT}/tools/ci/validate_dlt_monotonicity.py"
if [[ ! -f "${VALIDATOR}" ]]; then
...
LTICK_TRACE_JSONL="${EVIDENCE_DIR}/ltick_trace.jsonl"
...
python3 "${VALIDATOR}" \
  --eti-jsonl "${ETI_JSONL}" \
  --out-ltick-trace "${LTICK_TRACE_JSONL}" \
...
if [[ ! -f "${LTICK_TRACE_JSONL}" ]]; then
  echo "ERROR: validator did not produce ltick trace: ${LTICK_TRACE_JSONL}" >&2
  exit 3
fi
...
if [[ "${VALIDATOR_RC}" -ne 0 ]]; then
  ...
  echo "dlt-monotonicity: FAIL (${COUNT} violations)"
  exit 2
fi

echo "dlt-monotonicity: PASS"
exit 0
