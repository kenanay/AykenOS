...
#
# Validation-only Ring3 BCIB fixture execution evidence. Stub completion is
# explicitly disabled; the Ring3 worker closes work via public syscall 1011.
...
usage() {
  cat <<'USAGE'
Usage:
  scripts/ci/gate_execution_worker_completion.sh \
    --evidence-dir evidence/run-<id>/gates/execution-worker-completion \
    [--qemu-timeout <sec>]

Exit codes:
  0: pass
  1: tooling/build infrastructure failure
  2: Ring3 worker completion evidence contract failure
  3: usage error
USAGE
}

EVIDENCE_DIR=""
QEMU_TIMEOUT="${EXECUTION_WORKER_COMPLETION_QEMU_TIMEOUT:-35}"
...
VALIDATOR="${ROOT}/tools/ci/validate_execution_worker_completion.py"
BOOT_AUDIT_MARKER="[[AYKEN_BCIB_WORKER_USER_OBSERVED_OK]]"

if [[ ! -x "${BOOT_AUDIT}" || ! -f "${VALIDATOR}" ]]; then
  echo "ERROR: worker completion evidence dependency missing" >&2
  exit 1
fi
...
BUILD_ARGS=(
  ...
  USER_MINIMAL_MODE=bcib-worker-completion
  AYKEN_CR3_PCID=0
...
if [[ "${BUILD_RC}" -ne 0 ]]; then
  printf '%s\n' '{"gate":"execution-worker-completion","verdict":"FAIL","violations_count":1,"violations":["build_failed"]}' > "${REPORT_JSON}"
  echo "build_failed" > "${VIOLATIONS_TXT}"
  echo "execution-worker-completion: INFRA FAIL (build_failed)"
  exit 1
fi
...
{
  echo "gate=execution-worker-completion"
  echo "scope=validation_only_ring3_literal_fixture_public_complete_execution"
  echo "qemu_timeout_seconds=${QEMU_TIMEOUT}"
  echo "boot_audit_exit_code=${BOOT_AUDIT_RC}"
  echo "user_minimal_mode=bcib-worker-completion"
  echo "ayken_bcib_stub_result_enable=0"
  echo "ayken_bcib_worker_completion_selftest=1"
  ...
  echo "does_not_prove=general_bcib_interpreter,phase17_closure,race_isolation,performance_acceptance"
} > "${META_TXT}"

if [[ "${VALIDATOR_RC}" -ne 0 ]]; then
  echo "execution-worker-completion: FAIL (see ${REPORT_JSON})"
  exit 2
fi

echo "execution-worker-completion: PASS"
