...
usage() {
  cat <<'USAGE'
Usage:
  scripts/ci/gate_cross_node_parity.sh \
    --evidence-dir evidence/run-<id>/gates/cross-node-parity

Exit codes:
  0: pass
  2: cross-node parity gate failure
...
set +e
cargo run --quiet --manifest-path "${ROOT}/ayken-core/Cargo.toml" -p proof-verifier --example phase12_gate_harness -- cross-node-parity --out-dir "${EVIDENCE_DIR}"
HARNESS_RC=$?
set -e
...
if [[ "${HARNESS_RC}" -ne 0 ]]; then
  ...
  echo "cross-node-parity: FAIL (${COUNT} violations)"
  exit 2
fi

echo "cross-node-parity: PASS"
exit 0
