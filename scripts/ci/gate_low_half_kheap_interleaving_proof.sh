...
Usage:
  scripts/ci/gate_low_half_kheap_interleaving_proof.sh \
    --evidence-dir evidence/run-<id>/gates/low-half-kheap-interleaving-proof \
    [--qemu-timeout <sec>]

Environment:
  AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT=<n>  Integer >= 2 for the
                                                    validation-only overlap-pressure workload
...
AYKEN_LOW_HALF_KHEAP_MULTI_EXIT_PROOF_SELFTEST="${AYKEN_LOW_HALF_KHEAP_MULTI_EXIT_PROOF_SELFTEST:-0}"
AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_SELFTEST="${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_SELFTEST:-1}"
AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT="${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT:-2}"
EXPECTED_USER_MINIMAL_MODE="phase10a2"
OBSERVED_USER_MINIMAL_MODE="${USER_MINIMAL_MODE:-}"
BOOT_AUDIT_MARKER="[[AYKEN_LOW_HALF_KHEAP_INTERLEAVING_SELFTEST_OK]]"
SELFTEST_PREPARED_MARKER="[[AYKEN_LOW_HALF_KHEAP_INTERLEAVING_SELFTEST_PREPARED]]"
SELFTEST_ARMED_MARKER="[[AYKEN_LOW_HALF_KHEAP_INTERLEAVING_SELFTEST_ARMED]]"
SELFTEST_LINEAGE_MARKER="[[AYKEN_LOW_HALF_KHEAP_INTERLEAVING_LINEAGE]]"
SELFTEST_FAIL_MARKER="[[AYKEN_LOW_HALF_KHEAP_INTERLEAVING_SELFTEST_FAIL]]"
...
if [[ "${OBSERVED_USER_MINIMAL_MODE}" != "${EXPECTED_USER_MINIMAL_MODE}" ]]; then
  echo "ERROR: low-half-kheap-interleaving-proof requires USER_MINIMAL_MODE=${EXPECTED_USER_MINIMAL_MODE} (current=${OBSERVED_USER_MINIMAL_MODE:-unset})" >&2
  exit 3
fi
if [[ "${KERNEL_PROFILE}" != "validation" ]]; then
  echo "ERROR: low-half-kheap-interleaving-proof requires KERNEL_PROFILE=validation (current=${KERNEL_PROFILE})" >&2
  exit 3
fi
if [[ "${AYKEN_CR3_PCID}" != "0" ]]; then
  echo "ERROR: low-half-kheap-interleaving-proof requires AYKEN_CR3_PCID=0 (current=${AYKEN_CR3_PCID})" >&2
  exit 3
fi
if [[ "${AYKEN_MB_SELFTEST}" != "1" ]]; then
  echo "ERROR: low-half-kheap-interleaving-proof requires AYKEN_MB_SELFTEST=1 (current=${AYKEN_MB_SELFTEST})" >&2
  exit 3
fi
if [[ "${AYKEN_GATE4_POLICY_TEST}" != "0" ]]; then
  echo "ERROR: low-half-kheap-interleaving-proof requires AYKEN_GATE4_POLICY_TEST=0 (current=${AYKEN_GATE4_POLICY_TEST})" >&2
  exit 3
fi
if [[ "${AYKEN_LOW_HALF_KHEAP_EXIT_PROOF_SELFTEST}" != "0" ]]; then
  echo "ERROR: low-half-kheap-interleaving-proof requires AYKEN_LOW_HALF_KHEAP_EXIT_PROOF_SELFTEST=0 (current=${AYKEN_LOW_HALF_KHEAP_EXIT_PROOF_SELFTEST})" >&2
  exit 3
fi
if [[ "${AYKEN_LOW_HALF_KHEAP_MULTI_EXIT_PROOF_SELFTEST}" != "0" ]]; then
  echo "ERROR: low-half-kheap-interleaving-proof requires AYKEN_LOW_HALF_KHEAP_MULTI_EXIT_PROOF_SELFTEST=0 (current=${AYKEN_LOW_HALF_KHEAP_MULTI_EXIT_PROOF_SELFTEST})" >&2
  exit 3
fi
if [[ "${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_SELFTEST}" != "1" ]]; then
  echo "ERROR: low-half-kheap-interleaving-proof requires AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_SELFTEST=1 (current=${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_SELFTEST})" >&2
  exit 3
fi
if [[ ! "${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT}" =~ ^[1-9][0-9]*$ ]]; then
  echo "ERROR: low-half-kheap-interleaving-proof requires AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT to be an integer >= 2 (current=${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT})" >&2
  exit 3
fi
if (( AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT < 2 )); then
  echo "ERROR: low-half-kheap-interleaving-proof requires AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT >= 2 (current=${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT})" >&2
  exit 3
fi
...
CONTRACT_JSON="${EVIDENCE_DIR}/interleaving_contract.json"
...
make -C "${ROOT}" \
  ...
  AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT="${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT}" \
...
path = sys.argv[1]
row = {
    "gate": "low-half-kheap-interleaving-proof",
    "verdict": "FAIL",
    "violations_count": 1,
...
PY
  ...
  echo "low-half-kheap-interleaving-proof: INFRA FAIL (build_failed)"
  exit 1
fi
...
PY
  ...
  echo "low-half-kheap-interleaving-proof: FAIL (marker_log_empty)"
  exit 2
fi

python3 - "${MARKER_LOG}" "${CONTRACT_JSON}" "${LINEAGE_PIDS_TXT}" "${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT}" <<'PY'
...
PREPARED = "LOW_HALF_KHEAP_INTERLEAVING_SELFTEST_PREPARED]]"
ARMED = "LOW_HALF_KHEAP_INTERLEAVING_SELFTEST_ARMED]]"
LINEAGE = "LOW_HALF_KHEAP_INTERLEAVING_LINEAGE]]"
OK = "LOW_HALF_KHEAP_INTERLEAVING_SELFTEST_OK]]"
FAIL = "LOW_HALF_KHEAP_INTERLEAVING_SELFTEST_FAIL]]"
...
for line_no, raw in enumerate(marker_log.read_text(encoding="utf-8", errors="replace").splitlines(), start=1):
    if PREPARED in raw:
        fields = parse_fields(raw, PREPARED)
        try:
            ...
            fail_rows.append({"line": line_no, "raw_line": raw.strip(), "reason": "prepared_parse_failed"})
    if ARMED in raw:
    ...
    if OK in raw:
        ...
        try:
            ...
            ok_rows.append({
                ...
                "return_pid": parse_scalar(fields["return_pid"]),
                "exit_pids": exit_pids,
                "prepared_upfront": parse_scalar(fields["prepared_upfront"]),
                "raw_line": raw.strip(),
            })
        except (KeyError, ValueError):
...
prepared_rows.sort(key=lambda row: (row["slot"], row["line"]))
armed_rows.sort(key=lambda row: (row["slot"], row["line"]))
lineage_rows.sort(key=lambda row: (row["slot"], row["line"]))

violations: list[str] = []
ok_row = ok_rows[0] if len(ok_rows) == 1 else None
if len(ok_rows) != 1:
    violations.append(f"unexpected_interleaving_ok_marker_count:{len(ok_rows)}")
if ok_row is None:
    violations.append("missing_interleaving_ok_marker")

expected_total = ok_row["total"] if ok_row is not None else None
if expected_total is None and lineage_rows:
    expected_total = lineage_rows[0]["total"]
if expected_total is None and prepared_rows:
    expected_total = prepared_rows[0]["total"]
if expected_total is None and armed_rows:
    expected_total = armed_rows[0]["total"]
if expected_total is None:
    expected_total = 0

if expected_total <= 1:
    violations.append("invalid_expected_interleaving_total")
...
if len(prepared_rows) != expected_total:
    violations.append(f"unexpected_interleaving_prepared_count:{len(prepared_rows)}")
if len(armed_rows) != expected_total:
    violations.append(f"unexpected_interleaving_armed_count:{len(armed_rows)}")
if len(lineage_rows) != expected_total:
    violations.append(f"unexpected_interleaving_lineage_count:{len(lineage_rows)}")
if fail_rows:
    violations.append(f"interleaving_fail_markers_present:{len(fail_rows)}")

expected_slots = list(range(1, expected_total + 1))
if [row["slot"] for row in prepared_rows] != expected_slots:
    violations.append("interleaving_prepared_slots_noncanonical")
if [row["slot"] for row in armed_rows] != expected_slots:
    violations.append("interleaving_armed_slots_noncanonical")
if [row["slot"] for row in lineage_rows] != expected_slots:
    violations.append("interleaving_lineage_slots_noncanonical")

if prepared_rows and armed_rows and max(row["line"] for row in prepared_rows) >= min(row["line"] for row in armed_rows):
    violations.append("interleaving_prepared_not_before_first_armed")
if prepared_rows and lineage_rows and max(row["line"] for row in prepared_rows) >= min(row["line"] for row in lineage_rows):
    violations.append("interleaving_prepared_not_before_first_lineage")

prepared_by_slot = {row["slot"]: row for row in prepared_rows}
armed_by_slot = {row["slot"]: row for row in armed_rows}
lineage_by_slot = {row["slot"]: row for row in lineage_rows}
owner_values = set()
return_values = set()
selected_exit_pids = []
for slot in expected_slots:
    prepared = prepared_by_slot.get(slot)
    armed = armed_by_slot.get(slot)
    lineage = lineage_by_slot.get(slot)
    if prepared is None:
        violations.append(f"missing_interleaving_prepared_slot:{slot}")
        continue
    if armed is None:
        violations.append(f"missing_interleaving_armed_slot:{slot}")
        continue
    if lineage is None:
        violations.append(f"missing_interleaving_lineage_slot:{slot}")
        continue
    owner_values.update({prepared["owner_pid"], armed["owner_pid"], lineage["owner_pid"]})
    return_values.update({prepared["return_pid"], armed["return_pid"], lineage["return_pid"]})
    if prepared["total"] != expected_total or armed["total"] != expected_total or lineage["total"] != expected_total:
        violations.append(f"interleaving_total_mismatch:slot{slot}")
    if prepared["line"] >= armed["line"] or armed["line"] >= lineage["line"]:
        violations.append(f"interleaving_phase_order_invalid:slot{slot}")
    if prepared["exit_pid"] != armed["exit_pid"] or armed["exit_pid"] != lineage["exit_pid"]:
        violations.append(f"interleaving_exit_pid_mismatch:slot{slot}")
    if prepared["return_pid"] != armed["return_pid"] or armed["return_pid"] != lineage["return_pid"]:
        violations.append(f"interleaving_return_pid_mismatch:slot{slot}")
    if lineage["switch_from_pid"] != lineage["exit_pid"]:
        violations.append(f"interleaving_switch_from_mismatch:slot{slot}")
    if lineage["switch_to_pid"] != lineage["return_pid"]:
        violations.append(f"interleaving_switch_to_mismatch:slot{slot}")
    selected_exit_pids.append(lineage["exit_pid"])

if len(owner_values) > 1:
    violations.append("interleaving_owner_pid_inconsistent")
if len(return_values) > 1:
    violations.append("interleaving_return_pid_inconsistent")
if len(set(selected_exit_pids)) != len(selected_exit_pids):
    violations.append("interleaving_duplicate_exit_pid")
if ok_row is not None:
    if ok_row["prepared_upfront"] != 1:
        violations.append("interleaving_prepared_upfront_not_asserted")
    if ok_row["exit_pids"] != selected_exit_pids:
        violations.append("interleaving_ok_exit_pid_list_mismatch")
    if ok_row["owner_pid"] not in owner_values:
        violations.append("interleaving_ok_owner_pid_mismatch")
    if ok_row["return_pid"] not in return_values:
        violations.append("interleaving_ok_return_pid_mismatch")
...
contract = {
    ...
    "ok_rows": ok_rows,
    "fail_rows": fail_rows,
    "selected_exit_pids": selected_exit_pids,
    "prepared_upfront": bool(prepared_rows) and not any(
        item.startswith("interleaving_prepared_not_before_") for item in violations
    ),
...
python3 - "${REPORT_JSON}" "${CONTRACT_JSON}" "${LINEAGE_RESULTS_TXT}" "${BOOT_AUDIT_RC}" "${QEMU_TIMEOUT}" "${BOOT_AUDIT_MARKER}" "${SELFTEST_PREPARED_MARKER}" "${SELFTEST_ARMED_MARKER}" "${SELFTEST_LINEAGE_MARKER}" "${SELFTEST_FAIL_MARKER}" "${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT}" <<'PY'
from __future__ import annotations
...
selftest_prepared_marker = sys.argv[7]
selftest_armed_marker = sys.argv[8]
selftest_lineage_marker = sys.argv[9]
selftest_fail_marker = sys.argv[10]
configured_exit_count = int(sys.argv[11], 0)
...
if contract_path.is_file():
    ...
    violations.append("missing_interleaving_contract")
...
row = {
    ...
    "selftest_prepared_marker": selftest_prepared_marker,
...

{
  ...
  echo "ayken_low_half_kheap_interleaving_proof_selftest=${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_SELFTEST}"
  echo "ayken_low_half_kheap_interleaving_proof_count=${AYKEN_LOW_HALF_KHEAP_INTERLEAVING_PROOF_COUNT}"
...
PY
then
  echo "low-half-kheap-interleaving-proof: PASS"
  exit 0
fi

echo "low-half-kheap-interleaving-proof: FAIL"
exit 2
