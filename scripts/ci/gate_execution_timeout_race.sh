...
#
# Validation-only IRQ-timeout versus delayed Ring3 completion evidence. A
# bounded logical deadline is armed after delivery while Ring3 remains
# runnable; the public completion surface is unchanged and the stub is off.
...
usage() {
  cat <<'USAGE'
Usage:
  scripts/ci/gate_execution_timeout_race.sh \
    --evidence-dir evidence/run-<id>/gates/execution-timeout-race \
    [--qemu-timeout <sec>]

Exit codes:
  0: pass
  1: tooling/build infrastructure failure
  2: IRQ timeout race evidence contract failure
  3: usage error
USAGE
}

EVIDENCE_DIR=""
QEMU_TIMEOUT="${EXECUTION_TIMEOUT_RACE_QEMU_TIMEOUT:-35}"
...
VALIDATOR="${ROOT}/tools/ci/validate_execution_timeout_race.py"
BOOT_AUDIT_MARKER="[[AYKEN_EXEC_RACE_USER_OBSERVED_OK]]"

if [[ ! -x "${BOOT_AUDIT}" || ! -f "${VALIDATOR}" ]]; then
  echo "ERROR: timeout race evidence dependency missing" >&2
  exit 1
fi
...
BUILD_ARGS=(
  ...
  USER_MINIMAL_MODE=bcib-timeout-race
  AYKEN_CR3_PCID=0
...
if [[ "${BUILD_RC}" -ne 0 ]]; then
  printf '%s\n' '{"gate":"execution-timeout-race","verdict":"FAIL","violations_count":1,"violations":["build_failed"]}' > "${REPORT_JSON}"
  echo "build_failed" > "${VIOLATIONS_TXT}"
  echo "execution-timeout-race: INFRA FAIL (build_failed)"
  exit 1
fi
...
{
  echo "gate=execution-timeout-race"
  echo "scope=validation_only_delivered_running_deadline_irq_timeout_wins_over_delayed_public_completion"
  echo "qemu_timeout_seconds=${QEMU_TIMEOUT}"
  echo "boot_audit_exit_code=${BOOT_AUDIT_RC}"
  echo "user_minimal_mode=bcib-timeout-race"
  echo "ayken_bcib_stub_result_enable=0"
  echo "ayken_execution_race_selftest=1"
  ...
  echo "does_not_prove=exhaustive_race_matrix,smp_race_safety,general_bcib_interpreter,phase17_closure,performance_acceptance"
} > "${META_TXT}"

if [[ "${VALIDATOR_RC}" -ne 0 ]]; then
  echo "execution-timeout-race: FAIL (see ${REPORT_JSON})"
  exit 2
fi

echo "execution-timeout-race: PASS"
