...
usage() {
  cat <<'USAGE'
Usage:
  scripts/ci/gate_proof_verifier_cli.sh \
    --evidence-dir evidence/run-<id>/gates/proof-verifier-cli

Exit codes:
  0: pass
  2: proof verifier CLI gate failure
...
CLI_SMOKE_REPORT_JSON="${EVIDENCE_DIR}/cli_smoke_report.json"
...
cargo run --quiet --manifest-path "${ROOT}/ayken-core/Cargo.toml" \
  ...
  -- verifier-cli --out-dir "${EVIDENCE_DIR}" --cli-bin "${CLI_BIN}"
HARNESS_RC=$?
set -e

if [[ ! -f "${REPORT_JSON}" || ! -f "${CLI_SMOKE_REPORT_JSON}" || ! -f "${CLI_OUTPUT_CONTRACT_JSON}" ]]; then
  echo "ERROR: proof verifier CLI harness did not produce required outputs" >&2
  exit 3
fi
...
if [[ "${HARNESS_RC}" -ne 0 ]]; then
  ...
  echo "proof-verifier-cli: FAIL (${COUNT} violations)"
  exit 2
fi

echo "proof-verifier-cli: PASS"
exit 0
