...
- **Phase 10-A1**: Ring3 Process Preparation (COMPLETED - proc_create_user_process + queue)
- **Phase 10-A2**: Real CPL3 Entry Proof (IRETQ, CR3 switch, #BP marker)
- **Phase 10-B**: Full ELF Parsing (multi-segment, BSS, error handling)
- **Phase 10-C**: Process Integration (scheduler dispatch, syscalls)
...
- Implementation: `jump_to_ring3()` → `proc_create_user_process()` → queue
- Markers: `KERNEL_BEFORE_RING3`, `[[AYKEN_RING3_PREP_OK]]`
- Proof: Process registered + PROC_READY state + queued
- NO CPL3 entry yet (scheduler dispatch path not active)

**Phase 10-A2 (PLANNED):**
- Goal: Real CPL3 entry via IRETQ
- Implementation: Scheduler dispatch → `ring3_enter()` → IRETQ → #BP
- Markers: `P10_TSS_OK`, `P10_CR3_SWITCH`, `P10_RING3_ENTER`, `P10_RING3_USER_CODE`
- Proof: CPL3 execution confirmed via #BP exception handler
...
- `jump_to_ring3()` calls `proc_create_user_process()` with embedded ELF
- Process created, ELF loaded, address space mapped, queued as PROC_READY
- Invariants validated: registration, runnable state
- Markers emitted: `KERNEL_BEFORE_RING3`, `[[AYKEN_RING3_PREP_OK]]`
- NO CPL3 entry yet (scheduler dispatch not active in Phase 10-A1)

**Completed Tasks:**
- [x] 1. Create minimal Ring3 userspace program (userspace/minimal/)
- [x] 2. Create ELF embedding tool (tools/embed_elf.py)
- [x] 3. Implement minimal ELF validation (kernel/elf/parser.c - private helpers)
- [x] 4. Implement user address space creation (kernel/mm/paging.c)
- [x] 5. Implement minimal segment loading (kernel/proc/proc.c - load_user_image)
- [x] 6. Implement execution environment setup (stack, context)
- [x] 7. Implement process creation and queueing (proc_create_user_process)
- [x] 8. Integrate into kernel boot (jump_to_ring3 in kernel_late_init)
- [x] 9. CI gate for preparation validation (marker: [[AYKEN_RING3_PREP_OK]])

**Phase 10-A1 Proof:**
```
Markers: KERNEL_BEFORE_RING3 → [[AYKEN_RING3_PREP_OK]]
Invariants: proc_find_by_pid(pid) == proc && proc->state == PROC_READY
```
