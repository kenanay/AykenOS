use proof_verifier::cartel_correlation::{
    run_cartel_correlation_gate, CartelCorrelationGateConfig,
};
...
fn run() -> Result<GateVerdict, String> {
    ...
    let outcome = run_cartel_correlation_gate(&command)?;
    Ok(outcome.verdict)
}

fn parse_cli(args: Vec<OsString>) -> Result<Option<CartelCorrelationGateConfig>, String> {
    ...
    let policy_path =
        policy_path.unwrap_or_else(|| artifact_root.join("cartel_correlation_policy.json"));

    Ok(Some(CartelCorrelationGateConfig {
        ledger_path,
        policy_path,
        output_dir,
...
fn print_usage() {
    println!(
        "\
Usage:
  verifier-cartel-correlation --artifact-root <dir> --output-dir <dir> [--ledger <path>] [--policy <path>] [--window-runs <n>] [--window-seconds <seconds>]

Purpose:
  Evaluate Verification Diversity Ledger evidence against Stage-1 cartel correlation policy.

Defaults:
  --ledger defaults to <artifact-root>/verification_diversity_ledger.json
  --policy defaults to <artifact-root>/cartel_correlation_policy.json
"
    );
}
